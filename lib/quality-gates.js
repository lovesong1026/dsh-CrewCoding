...
export function taskAccessOf(task) {
    if (task?.access === 'read' || task?.access === 'write')
        return task.access;
...
export function qualityPlanningPrompt() {
    return [
        ...
        'halted means the human stopped the team; call crew_resume before creating more work. escalated means the automatic review loop hit its ceiling; that is not halt.',
    ].join(' ');
}
export function describeQualityLoop(team) {
    ...
    if (team.halted === true) {
        return {
            ...
            deliverable: false,
            summary: 'Team is halted. Call crew_resume with a reason before creating more work.',
        };
    }
    if (delivery.ok) {
        return {

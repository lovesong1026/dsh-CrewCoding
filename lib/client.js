window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Shared, demand-driven state for the Crew browser monitor. */
		...
		/**
		* Register one successful Crew card as a monitoring demand.
		*
		...
		* that session (e.g. a run_code-wrapped crew_create) without turning
		...
		*/
		...
		const ACTIVITY_STATE_URL = "/plugins/dsh-crew/state";
		const ACTIVITY_HALT_URL = "/plugins/dsh-crew/halt";
		/**
		...
		* later in that session (e.g. a run_code-wrapped crew_create) is
		...
		* uses the lead whale.
		* @module dsh-crew/client/artwork
		*/
		/** Artwork route prefix served by the plugin host half. */
		const ART_BASE = "/plugins/dsh-crew/assets/";
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:src/client/CrewCard.module.css.mjs
		const css$1 = ".FUFaIG_root{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:10px;flex-direction:column;gap:8px;width:100%;min-width:0;padding:10px 12px;display:flex}.FUFaIG_head{align-items:center;gap:8px;min-width:0;display:flex}.FUFaIG_leadAvatar{object-fit:contain;filter:drop-shadow(0 1px 1px #122d4833);background:0 0;border:0;border-radius:0;flex:none;width:30px;height:30px}.FUFaIG_teamName{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:0 auto;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.FUFaIG_memberCount{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;margin-left:auto;font-size:11px;line-height:16px}.FUFaIG_panelButton{border:1px solid var(--dsw-alias-line-strong);background:var(--dsw-alias-bg-module);color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;flex:none;padding:2px 8px;font-size:10.5px;font-weight:600;line-height:16px;transition:border-color .12s,color .12s}.FUFaIG_panelButton:hover{border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary)}.FUFaIG_panelButton:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.FUFaIG_members{flex-wrap:wrap;gap:6px;min-width:0;display:flex}.FUFaIG_member{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);max-width:160px;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;align-items:center;gap:5px;padding:3px 8px 3px 3px;font-size:11px;font-weight:500;line-height:16px;transition:border-color .12s,background-color .12s;display:inline-flex}.FUFaIG_member:hover{border-color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-bg-fill-neutral)}.FUFaIG_member:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.FUFaIG_memberArt{object-fit:contain;filter:drop-shadow(0 1px 1px #122d482e);background:0 0;border:0;border-radius:0;width:24px;height:24px}.FUFaIG_memberInitial{background:var(--dsw-alias-bg-fill-business);width:20px;height:20px;color:var(--dsw-alias-label-on-fill);border-radius:50%;justify-content:center;align-items:center;font-size:10px;font-weight:600;line-height:20px;display:inline-flex}.FUFaIG_memberName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}";
		const tagId$1 = "@nanmicoder/dsh-crew-coding/CrewCard.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$1) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@nanmicoder/dsh-crew-coding";
			tag.dataset.pluginCss = tagId$1;
			tag.textContent = css$1;
			document.head.appendChild(tag);
		}
		var CrewCard_module_css_default = {
			"head": "FUFaIG_head",
			"leadAvatar": "FUFaIG_leadAvatar",
			"member": "FUFaIG_member",
			"memberArt": "FUFaIG_memberArt",
			"memberCount": "FUFaIG_memberCount",
			"memberInitial": "FUFaIG_memberInitial",
			"memberName": "FUFaIG_memberName",
			"members": "FUFaIG_members",
			"panelButton": "FUFaIG_panelButton",
			"root": "FUFaIG_root",
			"teamName": "FUFaIG_teamName"
		};
		//#endregion
		//#region lib/client/CrewCard.js
		/**
		...
		*/
		...
		const OPEN_PANEL_EVENT = "crew:open-panel";
		...
		* (historical session review). */
		...
		function CrewCard({ node, openMember, sessionId, t }) {
			...
			return (0, react_jsx_runtime.jsxs)("section", {
				className: CrewCard_module_css_default.root,
				"data-crew-card": true,
				"data-team-id": resolved.teamId,
				children: [(0, react_jsx_runtime.jsxs)("header", {
					className: CrewCard_module_css_default.head,
					children: [
						(0, react_jsx_runtime.jsx)("img", {
							className: CrewCard_module_css_default.leadAvatar,
							src: LEAD_ART,
							alt: "",
							"aria-hidden": true
						}),
						(0, react_jsx_runtime.jsx)("span", {
							className: CrewCard_module_css_default.teamName,
							title: resolved.teamName,
							children: resolved.teamName
						}),
						(0, react_jsx_runtime.jsx)("span", {
							className: CrewCard_module_css_default.memberCount,
							children: t("card.memberCount", { count: resolved.members.length })
						}),
						(0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: CrewCard_module_css_default.panelButton,
							onClick: () => {
					...
					className: CrewCard_module_css_default.members,
					children: resolved.members.map((member) => (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						className: CrewCard_module_css_default.member,
						onClick: () => {
						...
						children: [memberArtUrl(member.name, member.role) !== null ? (0, react_jsx_runtime.jsx)("img", {
							className: CrewCard_module_css_default.memberArt,
							src: memberArtUrl(member.name, member.role) ?? "",
							alt: "",
							"aria-hidden": true
						}) : (0, react_jsx_runtime.jsx)("span", {
							className: CrewCard_module_css_default.memberInitial,
							children: member.name.trim().slice(0, 1).toUpperCase() || "?"
						}), (0, react_jsx_runtime.jsx)("span", {
							className: CrewCard_module_css_default.memberName,
		...
		const css = "html{--crew-panel-shift:420px}html[data-crew-panel-open] [data-phase=active]{box-sizing:border-box;padding-right:var(--crew-panel-shift)}._5QyWDq_badge,._5QyWDq_panel{--dsw-alias-line-normal:var(--dsw-static-neutral-bluish-150,#e7e9ee);--dsw-alias-line-strong:color-mix(in srgb, var(--dsw-static-neutral-bluish-200,#e1e5ee) 50%, var(--dsw-static-neutral-bluish-300,#cfd3d6));--dsw-alias-bg-module:var(--dsw-alias-bg-layer-1,#fff);--dsw-alias-bg-fill-neutral:var(--dsw-static-neutral-bluish-100,#eef0f4);--dsw-alias-bg-fill-business:var(--dsw-alias-state-business-primary,#4d6bfe);--dsw-alias-bg-fill-success:var(--dsw-alias-state-success-primary,#12a150);--dsw-alias-bg-fill-warning:var(--dsw-alias-state-warn-primary,#e08700);--dsw-alias-bg-fill-danger:var(--dsw-alias-state-error-primary,#e5484d);--dsw-alias-state-success:var(--dsw-alias-state-success-primary,#12a150);--dsw-alias-state-warning:var(--dsw-alias-state-warn-primary,#e08700);--dsw-alias-state-danger:var(--dsw-alias-state-error-primary,#e5484d);--dsw-alias-label-on-fill:var(--dsw-alias-label-primary-inverted,#fff)}._5QyWDq_badge{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 92%, transparent);backdrop-filter:blur(16px);height:34px;box-shadow:0 8px 28px color-mix(in srgb, var(--dsw-alias-label-primary) 14%, transparent);color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;align-items:center;gap:7px;padding:0 12px;font-size:12px;font-weight:600;line-height:20px;transition:border-color .15s,transform .12s;display:inline-flex;position:absolute;top:64px;right:18px}._5QyWDq_badge:hover{border-color:var(--dsw-alias-line-strong);transform:translateY(-1px)}._5QyWDq_badge:active{transform:translateY(0)scale(.98)}._5QyWDq_badge:focus-visible,._5QyWDq_iconButton:focus-visible,._5QyWDq_memberRow:focus-visible,._5QyWDq_membersToggle:focus-visible,._5QyWDq_sectionToggleTitle:focus-visible,._5QyWDq_dagNode:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._5QyWDq_badgeDot,._5QyWDq_panelDot{background:var(--dsw-alias-label-tertiary);border-radius:50%;width:7px;height:7px}._5QyWDq_badgeDot[data-busy=true],._5QyWDq_panelDot[data-busy=true]{background:var(--dsw-alias-state-business-primary);animation:1.25s ease-in-out infinite _5QyWDq_crewPulse}._5QyWDq_badgeCount,._5QyWDq_memberCount,._5QyWDq_teamStats,._5QyWDq_stageLabel,._5QyWDq_taskId{font-variant-numeric:tabular-nums}._5QyWDq_panel{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--dsw-alias-line-strong) 58%, transparent);background:color-mix(in srgb, var(--dsw-alias-bg-module) 95%, transparent);backdrop-filter:blur(20px)saturate(1.08);box-shadow:0 12px 32px color-mix(in srgb, var(--dsw-alias-label-primary) 12%, transparent), 0 32px 72px color-mix(in srgb, var(--dsw-alias-label-primary) 16%, transparent);will-change:transform;border-radius:16px;flex-direction:column;animation:.16s ease-out _5QyWDq_crewPanelIn;display:flex;position:absolute;top:0;left:0;overflow:hidden}._5QyWDq_panel[data-dragging],._5QyWDq_panel[data-resizing]{user-select:none;box-shadow:0 16px 38px color-mix(in srgb, var(--dsw-alias-label-primary) 14%, transparent), 0 36px 78px color-mix(in srgb, var(--dsw-alias-label-primary) 18%, transparent)}@keyframes _5QyWDq_crewPanelIn{0%{opacity:0}to{opacity:1}}@keyframes _5QyWDq_crewPulse{0%,to{opacity:.42}50%{opacity:1}}._5QyWDq_panelHead{border-bottom:1px solid var(--dsw-alias-line-normal);cursor:grab;touch-action:none;flex:none;justify-content:space-between;align-items:center;min-height:44px;padding:0 14px 0 16px;display:flex}._5QyWDq_panelHead:active,._5QyWDq_panel[data-dragging] ._5QyWDq_panelHead{cursor:grabbing}._5QyWDq_panel[data-compact] ._5QyWDq_panelHead{cursor:default;touch-action:auto}._5QyWDq_panelTitle{color:var(--dsw-alias-label-primary);align-items:center;gap:8px;font-size:14px;font-weight:600;line-height:20px;display:inline-flex}._5QyWDq_panelControls{flex:none;align-items:center;gap:2px;display:inline-flex}._5QyWDq_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:7px;justify-content:center;align-items:center;padding:0;transition:background-color .12s,color .12s,transform .12s;display:inline-flex}._5QyWDq_iconButton:hover{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}._5QyWDq_iconButton:active{transform:scale(.94)}._5QyWDq_iconButton[data-control=dock][data-mode=docked] svg{transform:scaleX(-1)}._5QyWDq_resizeHandle{z-index:1;touch-action:none;position:absolute}._5QyWDq_resizeHandle[data-resize-edge=left]{cursor:ew-resize;width:8px;top:44px;bottom:8px;left:0}._5QyWDq_resizeHandle[data-resize-edge=bottom]{cursor:ns-resize;height:8px;bottom:0;left:12px;right:12px}._5QyWDq_resizeHandle[data-resize-edge=corner]{cursor:nwse-resize;width:18px;height:18px;bottom:0;right:0}._5QyWDq_resizeHandle[data-resize-edge=corner]:after{border-right:1px solid var(--dsw-alias-label-tertiary);border-bottom:1px solid var(--dsw-alias-label-tertiary);content:\"\";opacity:.52;width:7px;height:7px;position:absolute;bottom:4px;right:4px}._5QyWDq_teams{overscroll-behavior:contain;scrollbar-color:color-mix(in srgb, var(--dsw-alias-label-tertiary) 28%, transparent) transparent;scrollbar-width:thin;flex-direction:column;min-height:0;display:flex;overflow-y:auto}._5QyWDq_teams::-webkit-scrollbar{width:6px}._5QyWDq_teams::-webkit-scrollbar-track{background:0 0}._5QyWDq_teams::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 28%, transparent);background-clip:padding-box;border:2px solid #0000;border-radius:999px}._5QyWDq_teams:hover::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 44%, transparent);background-clip:padding-box}._5QyWDq_team{border-bottom:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:12px;padding:12px 14px 16px;display:flex;container:_5QyWDq_agent-team/inline-size}._5QyWDq_team:last-child{border-bottom:0}._5QyWDq_teamHead{align-items:center;gap:10px;min-width:0;display:flex}._5QyWDq_teamName{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:13px;font-weight:600;line-height:18px;overflow:hidden}._5QyWDq_teamStats{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;gap:8px;font-size:10.5px;line-height:16px;display:inline-flex}._5QyWDq_teamStopButton{border:1px solid var(--dsw-alias-line-normal);width:26px;height:26px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border-radius:7px;flex:none;place-items:center;padding:0;transition:border-color .15s,background .15s,color .15s;display:grid}._5QyWDq_teamStopButton:hover{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 42%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-danger) 7%, transparent);color:var(--dsw-alias-state-danger)}._5QyWDq_teamStopButton:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._5QyWDq_stopModalActions{justify-content:flex-end;gap:8px;display:flex}._5QyWDq_stopModalActions button{border:1px solid var(--dsw-alias-line-normal,#e7e9ee);background:var(--dsw-alias-bg-fill-neutral,#eef0f4);min-height:34px;color:var(--dsw-alias-label-primary,#1c1c1e);cursor:pointer;font:inherit;border-radius:8px;justify-content:center;align-items:center;gap:6px;padding:6px 13px;font-size:12px;font-weight:600;display:inline-flex}._5QyWDq_stopModalActions button[data-danger]{border-color:var(--dsw-alias-state-danger,#e5484d);background:var(--dsw-alias-state-danger,#e5484d);color:var(--dsw-alias-label-on-fill,#fff)}._5QyWDq_stopModalActions button:disabled{cursor:wait;opacity:.58}._5QyWDq_stopModalError{background:color-mix(in srgb, var(--dsw-alias-state-danger,#e5484d) 8%, transparent);color:var(--dsw-alias-state-danger,#e5484d);border-radius:8px;align-items:flex-start;gap:7px;margin:0;padding:9px 10px;font-size:12px;line-height:18px;display:flex}._5QyWDq_stopModalError svg{flex:none;margin-top:1px}._5QyWDq_sectionHead{justify-content:space-between;align-items:center;gap:8px;min-width:0;display:flex}._5QyWDq_sectionTitle{color:var(--dsw-alias-label-secondary);align-items:center;gap:6px;font-size:11px;font-weight:600;line-height:16px;display:inline-flex}._5QyWDq_sectionHint{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}._5QyWDq_delegationSection{min-width:0}._5QyWDq_captainNode{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 32%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 7%, var(--dsw-alias-bg-module));border-radius:10px;grid-template-columns:48px minmax(0,1fr) auto;align-items:center;gap:9px;min-height:56px;padding:6px 10px;display:grid}._5QyWDq_captainAvatar,._5QyWDq_memberAvatar{flex:none;justify-content:center;align-items:center;display:inline-flex;position:relative}._5QyWDq_captainAvatar{width:46px;height:46px}._5QyWDq_leadAvatar,._5QyWDq_memberArt{object-fit:contain;filter:drop-shadow(0 1px 1px #122d4833);background:0 0;border:0;border-radius:0}._5QyWDq_leadAvatar{width:44px;height:44px}._5QyWDq_memberArt{width:40px;height:40px}._5QyWDq_captainInfo,._5QyWDq_memberInfo{flex-direction:column;min-width:0;display:flex}._5QyWDq_captainInfo{gap:2px}._5QyWDq_captainLine,._5QyWDq_memberLine{align-items:center;gap:6px;min-width:0;display:flex}._5QyWDq_captainName,._5QyWDq_memberName{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:12.5px;font-weight:600;line-height:18px;overflow:hidden}._5QyWDq_captainRole,._5QyWDq_memberRole{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}._5QyWDq_captainSummary,._5QyWDq_memberStatusLine{color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;font-size:10.5px;line-height:15px;overflow:hidden}._5QyWDq_memberModel,._5QyWDq_taskDetailModel{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9.5px;line-height:14px;overflow:hidden}._5QyWDq_captainState,._5QyWDq_memberState{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;align-items:center;gap:5px;font-size:10px;font-weight:500;line-height:15px;display:inline-flex}._5QyWDq_captainState[data-busy=true],._5QyWDq_memberState[data-activity=working]{color:var(--dsw-alias-state-business-primary)}._5QyWDq_workGlyph rect{opacity:.5}._5QyWDq_workGlyph[data-active=true] rect{animation:1.1s ease-in-out infinite _5QyWDq_crewDot}@keyframes _5QyWDq_crewDot{0%,to{opacity:.25}50%{opacity:1}}._5QyWDq_progressOverview{flex-direction:column;gap:7px;display:flex}._5QyWDq_progressTitle{color:var(--dsw-alias-label-secondary);font-size:11px;font-weight:600;line-height:16px}._5QyWDq_progressSegments{gap:3px;display:flex}._5QyWDq_progressSegments>span,._5QyWDq_progressEmpty{background:var(--dsw-alias-line-strong);border-radius:2px;flex:1;height:5px}._5QyWDq_progressEmpty{width:100%;display:block}._5QyWDq_progressSegments>span[data-state=running]{background:var(--dsw-alias-state-business-primary)}._5QyWDq_progressSegments>span[data-state=blocked]{background:var(--dsw-alias-state-warning)}._5QyWDq_progressSegments>span[data-state=completed]{background:var(--dsw-alias-state-success)}._5QyWDq_progressSegments>span[data-state=failed]{background:var(--dsw-alias-state-danger)}._5QyWDq_progressSegments>span[data-state=cancelled]{opacity:.55}._5QyWDq_progressLegend{color:var(--dsw-alias-label-tertiary);gap:10px;font-size:9.5px;line-height:14px;display:flex}._5QyWDq_progressLegend>span[data-state=running]{color:var(--dsw-alias-state-business-primary)}._5QyWDq_progressLegend>span[data-state=blocked]{color:var(--dsw-alias-state-warning)}._5QyWDq_progressLegend>span[data-state=completed]{color:var(--dsw-alias-state-success)}._5QyWDq_progressSummary{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 7%, var(--dsw-alias-bg-module));min-width:0;color:var(--dsw-alias-label-secondary);border-radius:8px;align-items:center;gap:6px;padding:5px 8px;font-size:10px;font-weight:600;line-height:15px;display:flex}._5QyWDq_progressSummary[data-state=warning]{background:color-mix(in srgb, var(--dsw-alias-state-warning) 8%, var(--dsw-alias-bg-module))}._5QyWDq_progressSummary[data-state=completed]{background:color-mix(in srgb, var(--dsw-alias-state-success) 8%, var(--dsw-alias-bg-module))}._5QyWDq_progressSummary[data-state=discarded]{background:var(--dsw-alias-bg-fill-neutral)}._5QyWDq_progressSummary>span:last-child{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._5QyWDq_progressSummaryDot{background:var(--dsw-alias-state-business-primary);border-radius:50%;flex:none;width:5px;height:5px}._5QyWDq_progressSummary[data-state=warning] ._5QyWDq_progressSummaryDot{background:var(--dsw-alias-state-warning)}._5QyWDq_progressSummary[data-state=completed] ._5QyWDq_progressSummaryDot{background:var(--dsw-alias-state-success)}._5QyWDq_progressSummary[data-state=discarded] ._5QyWDq_progressSummaryDot{background:var(--dsw-alias-label-tertiary)}._5QyWDq_membersToggle{background:var(--dsw-alias-bg-module-platform);width:100%;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border:0;border-radius:8px;justify-content:space-between;align-items:center;gap:8px;padding:6px 8px;font-size:10.5px;font-weight:600;line-height:15px;display:flex}._5QyWDq_membersToggle:hover{background:var(--dsw-alias-bg-fill-neutral)}._5QyWDq_membersToggle>span{align-items:center;gap:5px;display:inline-flex}._5QyWDq_membersToggle>span:last-child{color:var(--dsw-alias-state-business-primary)}._5QyWDq_chevron{flex:none;transition:transform .14s}._5QyWDq_chevron[data-open=true]{transform:rotate(90deg)}._5QyWDq_delegationTree{flex-direction:column;gap:2px;margin-left:18px;padding:9px 0 0 20px;display:flex;position:relative}._5QyWDq_delegationTree:before{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 48%, var(--dsw-alias-line-normal));content:\"\";width:1px;position:absolute;top:0;bottom:22px;left:0}._5QyWDq_memberBlock{flex-direction:column;min-width:0;padding:3px 0 7px;display:flex;position:relative}._5QyWDq_memberBranch{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 48%, var(--dsw-alias-line-normal));width:20px;height:1px;display:block;position:absolute;top:27px;right:100%}._5QyWDq_memberBranch:before{background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:50%;width:5px;height:5px;position:absolute;top:-2px;right:-1px}._5QyWDq_memberRow{box-sizing:border-box;width:100%;min-width:0;min-height:48px;color:inherit;font:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:8px;grid-template-columns:46px minmax(0,1fr) auto;align-items:center;gap:8px;padding:4px 6px;transition:background-color .12s,transform .12s;display:grid}._5QyWDq_memberRow:hover,._5QyWDq_memberRow[data-activity=working]{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 6%, var(--dsw-alias-bg-module))}._5QyWDq_memberRow:active{transform:scale(.995)}._5QyWDq_memberAvatar{width:42px;height:42px}._5QyWDq_memberAvatar[data-unread=true]:after{box-sizing:border-box;border:1px solid var(--dsw-alias-bg-module);background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:50%;width:6px;height:6px;animation:1.8s ease-in-out infinite _5QyWDq_crewUnreadPulse;position:absolute;top:0;right:-1px}@keyframes _5QyWDq_crewUnreadPulse{0%,to{opacity:.78;transform:scale(.92)}50%{opacity:1;transform:scale(1.16)}}._5QyWDq_memberInitial{background:var(--dsw-alias-bg-fill-business);width:34px;height:34px;color:var(--dsw-alias-label-on-fill);border-radius:50%;justify-content:center;align-items:center;font-size:14px;font-weight:600;line-height:20px;display:inline-flex}._5QyWDq_stateArt{box-sizing:border-box;object-fit:contain;width:22px;height:22px;filter:drop-shadow(0 0 1px var(--dsw-alias-bg-module)) drop-shadow(0 1px 1px #122d483d);background:0 0;border:0;border-radius:0;position:absolute;bottom:-3px;right:-5px}._5QyWDq_stateArt[data-activity=working]{animation:2.4s ease-in-out infinite _5QyWDq_crewFloat}._5QyWDq_stateArt[data-activity=idle]{animation:4.2s ease-in-out infinite _5QyWDq_crewBreathe}._5QyWDq_stateArt[data-activity=unknown]{animation:2.8s ease-in-out infinite _5QyWDq_crewThink}@keyframes _5QyWDq_crewFloat{0%,to{transform:translateY(0)rotate(-4deg)}50%{transform:translateY(-2px)rotate(4deg)}}@keyframes _5QyWDq_crewBreathe{0%,to{opacity:.82;transform:scale(1)}50%{opacity:1;transform:scale(1.06)}}@keyframes _5QyWDq_crewThink{0%,to{transform:rotate(-7deg)}50%{transform:rotate(7deg)}}._5QyWDq_memberState{margin-left:auto}._5QyWDq_memberCount{color:var(--dsw-alias-label-tertiary);font-size:10.5px;line-height:16px}._5QyWDq_assignmentLine{align-items:center;gap:7px;min-width:0;padding:0 6px 0 60px;display:flex}._5QyWDq_assignmentLabel{color:var(--dsw-alias-label-tertiary);flex:none;font-size:9.5px;line-height:14px}._5QyWDq_assignmentTasks{flex-wrap:wrap;flex:1;gap:4px;min-width:0;display:flex}._5QyWDq_assignmentChip{background:var(--dsw-alias-bg-fill-neutral);max-width:100%;min-height:16px;color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;border-radius:4px;align-items:center;padding:0 5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;font-weight:600;line-height:14px;display:inline-flex;overflow:hidden}._5QyWDq_assignmentChip[data-state=running]{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill)}._5QyWDq_assignmentChip[data-state=completed]{background:var(--dsw-alias-bg-fill-success);color:var(--dsw-alias-label-on-fill)}._5QyWDq_assignmentChip[data-state=blocked]{background:var(--dsw-alias-bg-fill-warning);color:var(--dsw-alias-label-on-fill)}._5QyWDq_assignmentChip[data-state=failed]{background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}._5QyWDq_assignmentChip[data-state=cancelled]{color:var(--dsw-alias-label-tertiary);text-decoration:line-through}._5QyWDq_unreadPill{color:var(--dsw-alias-state-business-primary);white-space:nowrap;flex:none;font-size:9.5px;font-weight:600;line-height:14px}._5QyWDq_taskEmpty{color:var(--dsw-alias-label-tertiary);font-size:9.5px;line-height:14px}._5QyWDq_dependencySection{border-top:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:7px;min-width:0;padding-top:10px;display:flex}._5QyWDq_sectionToggleTitle{color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;background:0 0;border:0;align-items:center;gap:6px;padding:0;font-size:11px;font-weight:600;line-height:16px;display:inline-flex}._5QyWDq_dagViewport{scrollbar-width:thin;min-width:0;padding:2px 0 4px;overflow-x:auto}._5QyWDq_dagCanvas{min-width:100%;position:relative}._5QyWDq_dagCanvas[data-layout=parallel]{flex-wrap:wrap;gap:8px;display:flex}._5QyWDq_dagCanvas[data-layout=parallel] ._5QyWDq_dagNode{flex:92px;min-width:92px;position:relative}._5QyWDq_dagEdges{pointer-events:none;position:absolute;inset:0;overflow:visible}._5QyWDq_dagEdges path{fill:none;stroke:var(--dsw-alias-line-strong);stroke-width:1px;transition:opacity .14s,stroke .14s,stroke-width .14s}._5QyWDq_dagEdges path[data-active=true]{stroke:var(--dsw-alias-state-business-primary);stroke-width:1.6px}._5QyWDq_dagEdges path[data-dimmed=true]{opacity:.24}._5QyWDq_dagNode{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);color:var(--dsw-alias-label-primary);font:inherit;text-align:left;cursor:pointer;border-radius:6px;flex-direction:column;justify-content:center;gap:1px;padding:0 6px;transition:border-color .14s,background-color .14s,opacity .14s;display:flex;position:absolute}._5QyWDq_dagNode:hover,._5QyWDq_dagNode[data-focused=true]{border-color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 6%, var(--dsw-alias-bg-module))}._5QyWDq_dagNode[data-dimmed=true]{opacity:.3}._5QyWDq_dagNode[data-state=running][data-dimmed=true]{opacity:.58}._5QyWDq_dagNode[data-state=completed]{border-color:color-mix(in srgb, var(--dsw-alias-state-success) 48%, var(--dsw-alias-line-normal))}._5QyWDq_dagNode[data-state=blocked]{border-color:color-mix(in srgb, var(--dsw-alias-state-warning) 52%, var(--dsw-alias-line-normal))}._5QyWDq_dagNode[data-state=failed]{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 56%, var(--dsw-alias-line-normal))}._5QyWDq_dagNodeHead{color:var(--dsw-alias-label-primary);align-items:center;gap:4px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9.5px;font-weight:700;display:flex}._5QyWDq_dagNodeDot{background:var(--dsw-alias-line-strong);border-radius:1.5px;flex:none;width:5px;height:5px}._5QyWDq_dagNode[data-state=running] ._5QyWDq_dagNodeDot{background:var(--dsw-alias-state-business-primary)}._5QyWDq_dagNode[data-state=running] ._5QyWDq_dagNodeHead{padding-right:12px}._5QyWDq_dagRunningState{width:9px;height:9px;color:var(--dsw-alias-state-business-primary);pointer-events:none;justify-content:center;align-items:center;display:inline-flex;position:absolute;top:4px;right:5px}._5QyWDq_dagRunningState ._5QyWDq_workGlyph{width:9px;height:9px}._5QyWDq_dagNode[data-state=blocked] ._5QyWDq_dagNodeDot{background:var(--dsw-alias-state-warning)}._5QyWDq_dagNode[data-state=completed] ._5QyWDq_dagNodeDot{background:var(--dsw-alias-state-success)}._5QyWDq_dagNode[data-state=failed] ._5QyWDq_dagNodeDot{background:var(--dsw-alias-state-danger)}._5QyWDq_dagNodeLabel{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:11px;overflow:hidden}._5QyWDq_taskDetail{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:9px;flex-direction:column;gap:3px;min-width:0;padding:7px 9px;display:flex}._5QyWDq_taskDetailHead{align-items:center;gap:6px;min-width:0;display:flex}._5QyWDq_taskDetailId{color:var(--dsw-alias-state-business-primary);flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;font-weight:700}._5QyWDq_taskDetailSubject{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:600;line-height:16px;overflow:hidden}._5QyWDq_taskDetailBadge{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;padding:0 5px;font-size:8.5px;font-weight:600;line-height:14px}._5QyWDq_taskDetailBadge[data-state=running]{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill)}._5QyWDq_taskDetailBadge[data-state=blocked]{background:var(--dsw-alias-bg-fill-warning);color:var(--dsw-alias-label-on-fill)}._5QyWDq_taskDetailBadge[data-state=completed]{background:var(--dsw-alias-bg-fill-success);color:var(--dsw-alias-label-on-fill)}._5QyWDq_taskDetailBadge[data-state=failed]{background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}._5QyWDq_taskDetailLine,._5QyWDq_taskDetailMeta{color:var(--dsw-alias-label-secondary);font-size:9.5px;line-height:14px}._5QyWDq_taskDetailMeta{color:var(--dsw-alias-label-tertiary)}._5QyWDq_emptyHint{color:var(--dsw-alias-label-tertiary);padding:10px 12px;font-size:11px;line-height:16px}._5QyWDq_planEditor{border:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 30%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 94%, var(--dsw-alias-state-business-primary));box-shadow:inset 0 1px 0 color-mix(in srgb, var(--dsw-alias-label-primary) 5%, transparent);border-radius:10px;flex-direction:column;gap:12px;margin:0 10px 12px;padding:12px;display:flex}._5QyWDq_planHeader>span{justify-content:space-between;align-items:center;gap:8px;display:flex}._5QyWDq_planHeader>span>span{flex-direction:column;gap:2px;min-width:0;display:flex}._5QyWDq_planHeader strong{color:var(--dsw-alias-label-primary);font-size:12px}._5QyWDq_planHeader small{color:var(--dsw-alias-label-secondary);font-size:9px;font-weight:500;line-height:13px}._5QyWDq_planHeader em{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill);border-radius:999px;flex:none;padding:1px 7px;font-size:9px;font-style:normal;line-height:16px}._5QyWDq_planHeader p{color:var(--dsw-alias-label-secondary);margin:5px 0 0;font-size:10px;line-height:15px}._5QyWDq_planFlow{grid-template-columns:repeat(3,minmax(0,1fr));margin:0;padding:0;list-style:none;display:grid}._5QyWDq_planFlow li{min-width:0;color:var(--dsw-alias-label-tertiary);align-items:center;gap:5px;font-size:9px;font-weight:600;line-height:14px;display:flex;position:relative}._5QyWDq_planFlow li:not(:last-child):after{background:var(--dsw-alias-line-normal);content:\"\";flex:1;min-width:8px;height:1px;margin-right:5px}._5QyWDq_planFlow li>span{border:1px solid var(--dsw-alias-line-normal);border-radius:50%;flex:none;place-items:center;width:18px;height:18px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;display:grid}._5QyWDq_planFlow li[data-active]{color:var(--dsw-alias-state-business-primary)}._5QyWDq_planFlow li[data-active]>span{border-color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 12%, transparent)}._5QyWDq_planSection{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:8px;overflow:hidden}._5QyWDq_planSectionToggle,._5QyWDq_planCardHeader{box-sizing:border-box;width:100%;color:var(--dsw-alias-label-primary);cursor:pointer;text-align:left;background:0 0;border:0}._5QyWDq_planSectionToggle{justify-content:space-between;align-items:center;gap:8px;min-height:42px;padding:7px 9px;display:flex}._5QyWDq_planSectionToggle:hover,._5QyWDq_planCardHeader:hover{background:color-mix(in srgb, var(--dsw-alias-bg-fill-neutral) 46%, transparent)}._5QyWDq_planSectionToggle>span{align-items:baseline;gap:7px;min-width:0;display:flex}._5QyWDq_planSectionToggle strong{font-size:10.5px}._5QyWDq_planSectionToggle small{color:var(--dsw-alias-label-tertiary);font-size:9px}._5QyWDq_planList{border-top:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:0;display:flex}._5QyWDq_planEmpty{color:var(--dsw-alias-label-tertiary);text-align:center;margin:0;padding:12px;font-size:10px}._5QyWDq_planCard{background:0 0;border:0;border-radius:0;min-width:0;margin:0;padding:0;display:block;position:relative}._5QyWDq_planCard+._5QyWDq_planCard{border-top:1px solid var(--dsw-alias-line-normal)}._5QyWDq_planCard[data-open=true]{background:color-mix(in srgb, var(--dsw-alias-bg-base) 62%, transparent)}._5QyWDq_planCardHeader{grid-template-columns:minmax(80px,.9fr) minmax(72px,1.15fr) auto 12px;align-items:center;gap:7px;min-height:40px;padding:6px 9px;display:grid}._5QyWDq_planCardIdentity{flex-direction:column;gap:1px;min-width:0;display:flex}._5QyWDq_planCardIdentity strong,._5QyWDq_planTaskSummary{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;font-weight:650;line-height:14px;overflow:hidden}._5QyWDq_planCardIdentity>span,._5QyWDq_planCardMeta{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:12px;overflow:hidden}._5QyWDq_planTaskId{background:var(--dsw-alias-bg-fill-neutral);width:max-content;color:var(--dsw-alias-label-secondary);border-radius:4px;padding:1px 5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:8.5px;font-weight:700;line-height:14px}._5QyWDq_planDirty{background:color-mix(in srgb, var(--dsw-alias-state-warning) 13%, transparent);color:var(--dsw-alias-state-warning);border-radius:999px;justify-self:end;padding:1px 5px;font-size:8px;font-style:normal;font-weight:650;line-height:14px}._5QyWDq_planChevron{color:var(--dsw-alias-label-tertiary);flex:none;transition:transform .18s cubic-bezier(.2,.7,.2,1)}._5QyWDq_planChevron[data-open=true]{transform:rotate(90deg)}._5QyWDq_planCardBody{flex-direction:column;gap:8px;padding:0 9px 9px;display:flex}._5QyWDq_planCardBody fieldset{border:0;flex-direction:column;gap:7px;min-width:0;margin:0;padding:0;display:flex}._5QyWDq_planCardBody label,._5QyWDq_planNewTask label{min-width:0;color:var(--dsw-alias-label-tertiary);flex-direction:column;flex:1;gap:4px;font-size:9px;display:flex}._5QyWDq_planCardBody label small{color:var(--dsw-alias-label-tertiary);font-size:8px;line-height:11px}._5QyWDq_planCard input,._5QyWDq_planCard textarea,._5QyWDq_planCard select,._5QyWDq_planNewTask input{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-base);width:100%;min-width:0;color:var(--dsw-alias-label-primary);font:inherit;border-radius:6px;outline:none;font-size:10.5px;line-height:16px;transition:border-color .16s,box-shadow .16s}._5QyWDq_planCard input,._5QyWDq_planCard select,._5QyWDq_planNewTask input{min-height:32px;padding:6px 8px}._5QyWDq_planCard textarea{resize:vertical;min-height:58px;padding:7px 8px}._5QyWDq_planCard input:focus-visible,._5QyWDq_planCard textarea:focus-visible,._5QyWDq_planCard select:focus-visible,._5QyWDq_planNewTask input:focus-visible{border-color:var(--dsw-alias-state-business-primary);box-shadow:0 0 0 2px color-mix(in srgb, var(--dsw-alias-state-business-primary) 16%, transparent)}._5QyWDq_planGrid{grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:6px;display:grid}._5QyWDq_planModelPicker{grid-template-columns:minmax(0,1fr);gap:5px;display:grid}._5QyWDq_planModelMenu{width:100%;display:flex}._5QyWDq_planModelTrigger{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);width:100%;min-height:38px;color:var(--dsw-alias-label-primary);cursor:pointer;text-align:left;border-radius:7px;justify-content:space-between;align-items:center;gap:8px;padding:7px 9px;transition:border-color .16s,background-color .16s,transform .12s;display:flex}._5QyWDq_planModelTrigger:hover:not(:disabled){border-color:var(--dsw-alias-border-l3);background:var(--dsw-alias-interactive-bg-hover)}._5QyWDq_planModelTrigger:active:not(:disabled){transform:translateY(1px)}._5QyWDq_planModelTrigger:focus-visible{border-color:var(--dsw-alias-state-business-primary);outline:2px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 16%, transparent);outline-offset:1px}._5QyWDq_planModelTrigger:disabled{cursor:wait;opacity:.64}._5QyWDq_planModelTriggerCopy{align-items:baseline;gap:6px;min-width:0;display:flex}._5QyWDq_planModelTriggerCopy strong,._5QyWDq_planModelTriggerCopy span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._5QyWDq_planModelTriggerCopy strong{color:var(--dsw-alias-label-primary);font-size:10px;font-weight:650;line-height:15px}._5QyWDq_planModelTriggerCopy span{color:var(--dsw-alias-label-tertiary);font-size:9px;line-height:14px}._5QyWDq_planModelMenuRow{grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:8px;width:100%;min-width:0;display:grid}._5QyWDq_planModelMenuRow>span:first-child{color:var(--dsw-alias-label-primary)}._5QyWDq_planModelMenuRow strong{color:var(--dsw-alias-label-tertiary);text-align:right;text-overflow:ellipsis;white-space:nowrap;font-weight:450;overflow:hidden}._5QyWDq_planModelMenuBack{align-items:center;gap:7px;display:inline-flex}._5QyWDq_planModelMenuBack svg{transform:rotate(180deg)}._5QyWDq_planModelEffortRow{flex-direction:column;align-items:flex-start;min-width:0;display:flex}._5QyWDq_planModelEffortRow small{width:100%;color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}._5QyWDq_planModelHint{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:12px;overflow:hidden}._5QyWDq_planModelNotice{background:color-mix(in srgb, var(--dsw-alias-state-warning) 9%, transparent);color:var(--dsw-alias-label-secondary);border-radius:6px;grid-column:1/-1;justify-content:space-between;align-items:center;gap:8px;padding:6px 7px;font-size:8.5px;line-height:12px;display:flex}._5QyWDq_planModelNotice button{color:var(--dsw-alias-state-business-primary);cursor:pointer;font:inherit;background:0 0;border:0;flex:none;padding:2px 6px;font-weight:650}._5QyWDq_planActions,._5QyWDq_planApproveRow,._5QyWDq_planNewTask,._5QyWDq_planConfirm,._5QyWDq_planApproveActions,._5QyWDq_planSecondaryActions{align-items:center;gap:7px;display:flex}._5QyWDq_planReviewActions{grid-template-columns:minmax(0,1fr);gap:6px;width:100%;display:grid}._5QyWDq_planSecondaryActions{grid-template-columns:minmax(0,1fr) auto;display:grid}._5QyWDq_planActions{justify-content:flex-end}._5QyWDq_planActions button,._5QyWDq_planNewTask button,._5QyWDq_planApproveRow button,._5QyWDq_planConfirm button{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-fill-neutral);min-height:30px;color:var(--dsw-alias-label-primary);cursor:pointer;border-radius:6px;flex:none;padding:5px 10px;font-size:9.5px;font-weight:600;transition:background .16s,border-color .16s,transform .16s}._5QyWDq_planActions button:hover:not(:disabled),._5QyWDq_planNewTask button:hover:not(:disabled),._5QyWDq_planApproveRow button:hover:not(:disabled),._5QyWDq_planConfirm button:hover:not(:disabled){border-color:var(--dsw-alias-label-tertiary)}._5QyWDq_planActions button:active:not(:disabled),._5QyWDq_planNewTask button:active:not(:disabled),._5QyWDq_planApproveRow button:active:not(:disabled),._5QyWDq_planConfirm button:active:not(:disabled){transform:scale(.98)}._5QyWDq_planActions button[data-danger],._5QyWDq_planConfirm button[data-danger]{color:var(--dsw-alias-state-danger)}._5QyWDq_planFeedback{min-width:0;color:var(--dsw-alias-label-secondary);flex:1;align-items:center;gap:5px;font-size:9px;line-height:13px;animation:.18s ease-out _5QyWDq_plan-feedback-in;display:inline-flex}._5QyWDq_planFeedback[data-tone=success]{color:var(--dsw-alias-state-success)}._5QyWDq_planFeedback[data-tone=error]{color:var(--dsw-alias-state-danger)}._5QyWDq_planFeedback>span{border:1px solid;border-radius:50%;flex:none;place-items:center;width:15px;height:15px;display:grid}._5QyWDq_planFeedback svg{width:11px;height:11px}@keyframes _5QyWDq_plan-feedback-in{0%{opacity:0;transform:translateY(-2px)}to{opacity:1;transform:translateY(0)}}._5QyWDq_planConfirm{border:1px solid color-mix(in srgb, var(--dsw-alias-state-danger) 30%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-danger) 7%, transparent);border-radius:7px;flex-wrap:wrap;justify-content:flex-end;padding:7px}._5QyWDq_planConfirm>span{min-width:140px;color:var(--dsw-alias-label-secondary);flex:1;font-size:9px;line-height:13px}._5QyWDq_planNewTask{align-items:flex-end}._5QyWDq_planNewTask label{gap:4px}._5QyWDq_planNewTask label>span{line-height:13px}._5QyWDq_planApproveRow{z-index:1;border:1px solid var(--dsw-alias-line-normal);background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 94%, transparent);min-height:50px;box-shadow:0 -5px 16px color-mix(in srgb, var(--dsw-alias-bg-base) 35%, transparent);backdrop-filter:blur(8px);border-radius:8px;flex-direction:column;justify-content:flex-end;align-items:stretch;margin:0 -4px -4px;padding:8px;position:sticky;bottom:0}._5QyWDq_planApproveRow[data-armed=true]{border-color:color-mix(in srgb, var(--dsw-alias-state-business-primary) 45%, var(--dsw-alias-line-normal))}._5QyWDq_planApproveRow[data-discard=true]{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 45%, var(--dsw-alias-line-normal))}._5QyWDq_planApproveCopy{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}._5QyWDq_planApproveCopy strong{color:var(--dsw-alias-label-primary);font-size:9.5px;line-height:13px}._5QyWDq_planApproveCopy small{color:var(--dsw-alias-label-tertiary);font-size:8.5px;line-height:12px}._5QyWDq_planApproveRow button{background:var(--dsw-alias-state-business-primary);min-height:32px;color:var(--dsw-alias-label-on-fill);padding-inline:13px}._5QyWDq_planReviewActions>button[data-plan-approve]{width:100%}._5QyWDq_planApproveActions>button:first-child{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}._5QyWDq_planSecondaryActions>button,._5QyWDq_planApproveActions>button[data-danger]{border-color:var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}._5QyWDq_planSecondaryActions>button[data-danger],._5QyWDq_planApproveActions>button[data-danger]{color:var(--dsw-alias-state-danger)}._5QyWDq_planSectionToggle:focus-visible,._5QyWDq_planCardHeader:focus-visible,._5QyWDq_planActions button:focus-visible,._5QyWDq_planNewTask button:focus-visible,._5QyWDq_planApproveRow button:focus-visible,._5QyWDq_planConfirm button:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:-2px}._5QyWDq_planActions button:disabled,._5QyWDq_planNewTask button:disabled,._5QyWDq_planApproveRow button:disabled{cursor:default;opacity:.55}._5QyWDq_historicPill{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;margin-left:auto;padding:1px 7px;font-size:9.5px;font-weight:600;line-height:15px}._5QyWDq_members{flex-direction:column;gap:3px;display:flex}._5QyWDq_archiveLabel{color:var(--dsw-alias-label-tertiary);padding:5px 14px 0;font-size:9.5px;font-weight:600;line-height:14px;display:block}@media (prefers-reduced-motion:reduce){._5QyWDq_panel,._5QyWDq_badge,._5QyWDq_badgeDot,._5QyWDq_panelDot,._5QyWDq_workGlyph rect,._5QyWDq_stateArt,._5QyWDq_memberAvatar[data-unread=true]:after,._5QyWDq_planChevron,._5QyWDq_planFeedback,._5QyWDq_planActions button,._5QyWDq_planNewTask button,._5QyWDq_planApproveRow button,._5QyWDq_planConfirm button,._5QyWDq_planCard input,._5QyWDq_planCard textarea,._5QyWDq_planCard select,._5QyWDq_planNewTask input{transition:none;animation:none}}@media (width<=960px){html[data-crew-panel-open] [data-phase=active]{padding-right:0}}@media (width<=640px){._5QyWDq_badge{top:56px;right:10px}._5QyWDq_teamStats span[data-stat=messages]{display:none}._5QyWDq_captainNode{grid-template-columns:48px minmax(0,1fr)}._5QyWDq_captainState{display:none}._5QyWDq_delegationTree{margin-left:12px;padding-left:15px}._5QyWDq_memberBranch{width:15px}._5QyWDq_assignmentLine{padding-left:53px}._5QyWDq_planFlow li{gap:4px;font-size:8px}._5QyWDq_planFlow li:not(:last-child):after{margin-right:3px}._5QyWDq_planCardHeader{grid-template-columns:auto minmax(0,1fr) auto}._5QyWDq_planCardHeader ._5QyWDq_planCardMeta{display:none}._5QyWDq_planGrid,._5QyWDq_planModelPicker{grid-template-columns:minmax(0,1fr)}._5QyWDq_planNewTask,._5QyWDq_planApproveRow{flex-direction:column;align-items:stretch}._5QyWDq_planNewTask button,._5QyWDq_planApproveRow>button,._5QyWDq_planApproveActions,._5QyWDq_planReviewActions{width:100%}._5QyWDq_planApproveActions button,._5QyWDq_planReviewActions button,._5QyWDq_planSecondaryActions button{flex:1}}@container _5QyWDq_agent-team (width<=360px){._5QyWDq_planEditor{margin-inline:0;padding-inline:10px}._5QyWDq_planHeader>span{align-items:flex-start}._5QyWDq_planFlow li{gap:3px;font-size:7.5px}._5QyWDq_planFlow li:not(:last-child):after{min-width:4px;margin-right:2px}._5QyWDq_planSecondaryActions,._5QyWDq_planApproveActions{grid-template-columns:minmax(0,1fr);width:100%;display:grid}._5QyWDq_planSecondaryActions button,._5QyWDq_planApproveActions button{width:100%}}";
		const tagId = "@nanmicoder/dsh-crew-coding/ActivityPanel.module.css";
		...
		var ActivityPanel_module_css_default = {
			"agent-team": "_5QyWDq_agent-team",
			"archiveLabel": "_5QyWDq_archiveLabel",
			"assignmentChip": "_5QyWDq_assignmentChip",
			"assignmentLabel": "_5QyWDq_assignmentLabel",
			"assignmentLine": "_5QyWDq_assignmentLine",
			"assignmentTasks": "_5QyWDq_assignmentTasks",
			"badge": "_5QyWDq_badge",
			"badgeCount": "_5QyWDq_badgeCount",
			"badgeDot": "_5QyWDq_badgeDot",
			"captainAvatar": "_5QyWDq_captainAvatar",
			"captainInfo": "_5QyWDq_captainInfo",
			"captainLine": "_5QyWDq_captainLine",
			"captainName": "_5QyWDq_captainName",
			"captainNode": "_5QyWDq_captainNode",
			"captainRole": "_5QyWDq_captainRole",
			"captainState": "_5QyWDq_captainState",
			"captainSummary": "_5QyWDq_captainSummary",
			"chevron": "_5QyWDq_chevron",
			"crewBreathe": "_5QyWDq_crewBreathe",
			"crewDot": "_5QyWDq_crewDot",
			"crewFloat": "_5QyWDq_crewFloat",
			"crewPanelIn": "_5QyWDq_crewPanelIn",
			"crewPulse": "_5QyWDq_crewPulse",
			"crewThink": "_5QyWDq_crewThink",
			"crewUnreadPulse": "_5QyWDq_crewUnreadPulse",
			"dagCanvas": "_5QyWDq_dagCanvas",
			"dagEdges": "_5QyWDq_dagEdges",
			"dagNode": "_5QyWDq_dagNode",
			"dagNodeDot": "_5QyWDq_dagNodeDot",
			"dagNodeHead": "_5QyWDq_dagNodeHead",
			"dagNodeLabel": "_5QyWDq_dagNodeLabel",
			"dagRunningState": "_5QyWDq_dagRunningState",
			"dagViewport": "_5QyWDq_dagViewport",
			"delegationSection": "_5QyWDq_delegationSection",
			"delegationTree": "_5QyWDq_delegationTree",
			"dependencySection": "_5QyWDq_dependencySection",
			"emptyHint": "_5QyWDq_emptyHint",
			"historicPill": "_5QyWDq_historicPill",
			"iconButton": "_5QyWDq_iconButton",
			"leadAvatar": "_5QyWDq_leadAvatar",
			"memberArt": "_5QyWDq_memberArt",
			"memberAvatar": "_5QyWDq_memberAvatar",
			"memberBlock": "_5QyWDq_memberBlock",
			"memberBranch": "_5QyWDq_memberBranch",
			"memberCount": "_5QyWDq_memberCount",
			"memberInfo": "_5QyWDq_memberInfo",
			"memberInitial": "_5QyWDq_memberInitial",
			"memberLine": "_5QyWDq_memberLine",
			"memberModel": "_5QyWDq_memberModel",
			"memberName": "_5QyWDq_memberName",
			"memberRole": "_5QyWDq_memberRole",
			"memberRow": "_5QyWDq_memberRow",
			"memberState": "_5QyWDq_memberState",
			"memberStatusLine": "_5QyWDq_memberStatusLine",
			"members": "_5QyWDq_members",
			"membersToggle": "_5QyWDq_membersToggle",
			"panel": "_5QyWDq_panel",
			"panelControls": "_5QyWDq_panelControls",
			"panelDot": "_5QyWDq_panelDot",
			"panelHead": "_5QyWDq_panelHead",
			"panelTitle": "_5QyWDq_panelTitle",
			"plan-feedback-in": "_5QyWDq_plan-feedback-in",
			"planActions": "_5QyWDq_planActions",
			"planApproveActions": "_5QyWDq_planApproveActions",
			"planApproveCopy": "_5QyWDq_planApproveCopy",
			"planApproveRow": "_5QyWDq_planApproveRow",
			"planCard": "_5QyWDq_planCard",
			"planCardBody": "_5QyWDq_planCardBody",
			"planCardHeader": "_5QyWDq_planCardHeader",
			"planCardIdentity": "_5QyWDq_planCardIdentity",
			"planCardMeta": "_5QyWDq_planCardMeta",
			"planChevron": "_5QyWDq_planChevron",
			"planConfirm": "_5QyWDq_planConfirm",
			"planDirty": "_5QyWDq_planDirty",
			"planEditor": "_5QyWDq_planEditor",
			"planEmpty": "_5QyWDq_planEmpty",
			"planFeedback": "_5QyWDq_planFeedback",
			"planFlow": "_5QyWDq_planFlow",
			"planGrid": "_5QyWDq_planGrid",
			"planHeader": "_5QyWDq_planHeader",
			"planList": "_5QyWDq_planList",
			"planModelEffortRow": "_5QyWDq_planModelEffortRow",
			"planModelHint": "_5QyWDq_planModelHint",
			"planModelMenu": "_5QyWDq_planModelMenu",
			"planModelMenuBack": "_5QyWDq_planModelMenuBack",
			"planModelMenuRow": "_5QyWDq_planModelMenuRow",
			"planModelNotice": "_5QyWDq_planModelNotice",
			"planModelPicker": "_5QyWDq_planModelPicker",
			"planModelTrigger": "_5QyWDq_planModelTrigger",
			"planModelTriggerCopy": "_5QyWDq_planModelTriggerCopy",
			"planNewTask": "_5QyWDq_planNewTask",
			"planReviewActions": "_5QyWDq_planReviewActions",
			"planSecondaryActions": "_5QyWDq_planSecondaryActions",
			"planSection": "_5QyWDq_planSection",
			"planSectionToggle": "_5QyWDq_planSectionToggle",
			"planTaskId": "_5QyWDq_planTaskId",
			"planTaskSummary": "_5QyWDq_planTaskSummary",
			"progressEmpty": "_5QyWDq_progressEmpty",
			"progressLegend": "_5QyWDq_progressLegend",
			"progressOverview": "_5QyWDq_progressOverview",
			"progressSegments": "_5QyWDq_progressSegments",
			"progressSummary": "_5QyWDq_progressSummary",
			"progressSummaryDot": "_5QyWDq_progressSummaryDot",
			"progressTitle": "_5QyWDq_progressTitle",
			"resizeHandle": "_5QyWDq_resizeHandle",
			"sectionHead": "_5QyWDq_sectionHead",
			"sectionHint": "_5QyWDq_sectionHint",
			"sectionTitle": "_5QyWDq_sectionTitle",
			"sectionToggleTitle": "_5QyWDq_sectionToggleTitle",
			"stageLabel": "_5QyWDq_stageLabel",
			"stateArt": "_5QyWDq_stateArt",
			"stopModalActions": "_5QyWDq_stopModalActions",
			"stopModalError": "_5QyWDq_stopModalError",
			"taskDetail": "_5QyWDq_taskDetail",
			"taskDetailBadge": "_5QyWDq_taskDetailBadge",
			"taskDetailHead": "_5QyWDq_taskDetailHead",
			"taskDetailId": "_5QyWDq_taskDetailId",
			"taskDetailLine": "_5QyWDq_taskDetailLine",
			"taskDetailMeta": "_5QyWDq_taskDetailMeta",
			"taskDetailModel": "_5QyWDq_taskDetailModel",
			"taskDetailSubject": "_5QyWDq_taskDetailSubject",
			"taskEmpty": "_5QyWDq_taskEmpty",
			"taskId": "_5QyWDq_taskId",
			"team": "_5QyWDq_team",
			"teamHead": "_5QyWDq_teamHead",
			"teamName": "_5QyWDq_teamName",
			"teamStats": "_5QyWDq_teamStats",
			"teamStopButton": "_5QyWDq_teamStopButton",
			"teams": "_5QyWDq_teams",
			"unreadPill": "_5QyWDq_unreadPill",
			"workGlyph": "_5QyWDq_workGlyph"
		};
		//#endregion
		...
		*/
		const PLAN_URL = "/plugins/dsh-crew/plan";
		...
		*/
		...
		/** Pure persisted geometry rules for the Crew shell-overlay panel. */
		const PANEL_LAYOUT_STORAGE_KEY = "dsh-crew:activity-panel:v1";
		...
		/**
		* Crew activity panel: the top-right floater monitoring every team.
		*
		...
		* polls the host `/plugins/dsh-crew/state` route for
		...
		* @module dsh-crew/client/activity
		*/
		...
		*/
		...
		const PANEL_OPEN_ATTRIBUTE = "data-crew-panel-open";
		/** Shared width concession consumed by the conversation root CSS. */
		const PANEL_SHIFT_PROPERTY = "--crew-panel-shift";
		...
		* durable roster through the same rebuilt panel instead of a second UI. */
		...
		//#endregion
		//#region lib/client/crew-card-definition.js
		/**
		...
		*/
		...
		function parseCrewCreateArgs(value) {
			try {
				const parsed = JSON.parse(value);
		...
		const crewCardDefinition = {
			kind: "crew",
			target: "chat",
			match: (event) => {
				if (event.type === "tool/call" && event.data.name === "crew_create") return parseCrewCreateArgs(event.data.arguments) === void 0 ? null : {
			...
			start: (_context, match) => {
				if (match.event.type !== "tool/call") throw new Error("crew card start requires crew_create tool/call");
				const parsed = parseCrewCreateArgs(match.event.data.arguments);
				if (parsed === void 0) throw new Error("crew card start requires valid create arguments");
				return {
					...parsed,
					accepted: false
				};
		...
		/** `crew` namespace dictionaries for every plugin-owned Web surface. */
		/** Dictionary namespace owned by the Crew client plugin. */
		const CREW_LOCALE_NAMESPACE = "crew";
		/** Simplified Chinese dictionary (the key-set source of truth). */
		const zh = {
			"card.memberCount": "{count} 名成员",
			"action.openActivityPanel": "打开活动面板",
			"activity.panelButton": "活动面板",
			"activity.badgeAria": "Crew 活动与历史，{count} 条团队记录",
			"activity.panelAria": "Crew 活动面板",
			"activity.title": "Crew 活动",
		...
		const en = {
			...
			"activity.badgeAria": "Crew activity and history, {count} team records",
			"activity.panelAria": "Crew activity panel",
			"activity.title": "Crew activity",
		...
		/** Version-tolerant navigation into durable Crew member transcripts. */
		/**
		...
		*/
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(CREW_LOCALE_NAMESPACE, {
				zh,
				en
			}), "crew: dictionaries");

...
import { haltTeamWork, registerCrewTools, } from "./tools.js";
import { installCrewGestureBoundary, registerCrewCommand } from "./command.js";
...
export const Config = z.object({
    stateDir: z.string().default('.crew'),
...
4. After approval, the final member configuration is spawned atomically and the scheduler starts ready work. Lead by delegation: monitor with crew_status, send guidance with crew_send_message, and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. If the user requires every member to contribute or report, create one task per required contribution (or message each member directly); never wait for an unassigned member to produce work it was never given.
5. If the user explicitly asks to pause a running member, its open attempt remains parked after interruption; after answering the user, send that same member guidance with crew_send_message so it continues the same attempt. Do not interrupt members for an ordinary user question that did not request a pause. If work must change owner, restart from scratch, or be taken over, call crew_reassign_task first. Prefer another idle member or a retry with the same member. Use assignee=captain only for one ready task that you will personally drive to a terminal status in this same turn; never start a second captain takeover while one is unfinished, and never end your turn with captain-owned work open. Reassignment revokes the old attempt and waits for that member to quiesce, preventing late results from overwriting the new attempt.
...
export function apply(ctx, config) {
    ...
    if (config.slashCommand ?? true) {
        ctx.inject(['commands'], (commandCtx) => {
            registerCrewCommand(commandCtx, () => config.profiles ?? {});
        });
        installCrewGestureBoundary(ctx, () => config.profiles ?? {});
    }
    ...
    const registerWebSurface = () => {
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    ctx.logger.warn(`crew: halt failed for ${teamId}: ${String(error)}`);
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                ...
                try {
                    if (action === 'approve') {
                        const approved = await crewRuntime.approveStagedTeam(captain, teamId);
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                        res.end(JSON.stringify({ ok: true, phase: 'running', ...approved }));
                        return;
                    }
                    if (action === 'continue') {
                        const continued = await crewRuntime.continueStagedPlanning(captain, teamId);
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                        res.end(JSON.stringify({ ok: true, phase: 'staged', review: 'awaiting_feedback', ...continued }));
                        return;
                    }
                    if (action === 'discard') {
                        const discarded = await crewRuntime.discardStagedTeam(captain, teamId);
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    ...
                    const updated = await crewRuntime.updateStagedPlan(captain, teamId, mutation);
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
        ...
        const artDir = fileURLToPath(new URL('../assets/crew/', import.meta.url));
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    ctx.logger.warn(`crew: artwork read failed for ${name}: ${String(error)}`);
                    res.writeHead(404);
                    res.end();
                }
            },

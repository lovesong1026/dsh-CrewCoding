...
function requireCaptain(exec) {
    if (!exec.agent) {
        throw new Error('crew tools require a calling agent (exec.agent was undefined)');
    }
    return exec.agent;
}
...
async function requireCaptainTeam(workspace, config, captain) {
    ...
    if (team === undefined) {
        throw new Error('you are not leading any team yet — call crew_create first');
    }
    return team;
}
...
function requireTask(team, taskId) {
    ...
    if (task === undefined) {
        throw new Error(`no task "${taskId}" in team "${team.name}" — use crew_status to list tasks`);
    }
    return task;
}
...
async function stopTeamMemberActivations(ctx, captain, members, signal) {
    ...
    if (runtime.drainContinuableChildren !== undefined) {
        try {
            ...
            ctx.logger.warn(`crew: failed to drain halted members: ${String(error)}`);
            throw error;
        }
    }
...
export async function haltTeamWork(input) {
    const halted = await withTeamLock(teamLockKey(input.stateRoot, input.teamId), async () => {
        ...
        appendTeamEvent(input.ctx, captainSessionOf(input.ctx, fresh.captainSessionId, input.captain.session), 'crew/team-halted', {
            teamId: fresh.id,
            cancelledTasks,
        });
        return {
...
export function steerCaptainReport(captain, from, content) {
    try {
        captain.steer(createUserMessage({
            content: [{ type: 'text', text: `Crew message from member ${from}:\n\n${content}` }],
            source: { kind: 'plugin', plugin: 'dsh-crew' },
        }));
        return true;
    }
    catch {
...
export function stagedPlanDiscardContext(teamName) {
    return [
        `The user discarded the staged Crew plan "${teamName}" from the pre-run review UI.`,
        'That decision is final for this draft: it has been archived, no members were created, and no tasks may run.',
        'Do not call crew_create, crew_approve, or recreate a replacement team merely because the old team is no longer active.',
        'Wait for a later explicit user request. If the next user message is unrelated to Crew, answer it normally and do not start a team.',
    ].join('\n');
}
...
export function stagedPlanFeedbackContext(teamName) {
    return [
        `The user selected "Return to chat and revise" for the staged Crew plan "${teamName}".`,
        ...
        'After the user answers, revise this same staged roster and DAG with one atomic crew_edit_plan call, summarize the changes, and ask the user to review the updated plan again.',
    ].join('\n');
}
/**
 * Register every `crew_*` tool into the shared tools registry.
 * @param ctx - the plugin context (injects `tools`).
 * @param config - resolved tool config.
 */
export function registerCrewTools(ctx, config) {
    ...
    const approveStagedTeam = async (captain, teamId, signal) => {
        ...
        try {
            ...
            ctx.logger.warn(`crew: post-approval kick failed for "${teamId}": ${String(error)}`);
        }
        return approved;
    };
    ...
    const discardStagedTeam = async (captain, teamId) => {
        ...
        const discarded = await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
            const fresh = await requireFreshCaptainTeam(stateRoot, teamId, captain.id);
            requireStagedTeam(fresh);
            appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'crew/plan-discarded', {
                teamId: fresh.id,
            });
        ...
        try {
            ...
            ctx.logger.warn(`crew: failed to inject discard context for "${discarded.teamId}": ${String(error)}`);
        }
        captain.cancel({ kind: 'user' }, { keepInbox: true });
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (args.profile !== undefined && profileName === '') {
                throw new Error('Crew profile name must not be empty');
            }
            ...
            if (created.committed) {
                try {
                    ...
                    ctx.logger.warn(`crew: post-create kick failed for "${created.state.id}": ${String(error)}`);
                }
                try {
                    appendTeamEvent(ctx, captain.session, 'crew/team-created', {
                    ...
                    for (const member of created.state.members) {
                        appendTeamEvent(ctx, captain.session, 'crew/member-added', {
                            teamId: created.state.id,
                            memberId: member.id,
                            name: member.name,
                    ...
                    for (const task of created.state.tasks) {
                        appendTeamEvent(ctx, captain.session, 'crew/task-created', {
                            teamId: created.state.id,
                            taskId: task.id,
                    ...
                    ctx.logger.warn(`crew: post-create events failed for "${created.state.id}": ${String(error)}`);
                }
            }
    ...
    ctx.tools.register(defineTool({
        name: 'crew_edit_plan',
        description: 'Atomically revise the current staged Crew plan without spawning members or scheduling tasks. Use this when the user continues chatting to change a plan that is waiting for approval. Submit dependent edits in order (update downstream dependencies or assignees, then remove tasks, then remove unused members). Never inspect or edit .crew state files or plugin source code to revise a plan.',
        parameters: {
            operations: {
                type: 'array',
                required: true,
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const created = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'crew/member-added', {
                    teamId: fresh.id,
                    memberId: member.id,
                    name: member.name,
                    ...member.role !== undefined ? { role: member.role } : {},
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const revoked = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                await writeTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'crew/member-removed', {
                    teamId: fresh.id,
                    memberId: member.id,
                });
            ...
            const created = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (fresh.halted === true) {
                    ...
                    if (resumed.status !== 'resumed' || resumed.team === undefined) {
                        throw new Error(resumed.error ?? 'team is halted; call crew_resume or pass resume=true with resumeReason');
                    }
                    fresh.halted = false;
                    fresh.haltedAt = undefined;
                    appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'crew/team-resumed', {
                        teamId: fresh.id,
                ...
                await writeTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'crew/task-created', {
                    teamId: fresh.id,
                    taskId: task.id,
                    subject: task.subject,
                    dependencies: task.dependencies,
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                await writeTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captain.session, 'crew/task-updated', {
                    teamId: fresh.id,
                    taskId: task.id,
                    status: task.status,
            ...
            return withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (taskAccessOf(task) === 'write') {
                    ...
                    if (activeWriter !== undefined)
                        throw new Error('workspace write lease is held by ' + activeWriter.id + '; wait for it to finish before claiming ' + task.id);
                }
                const attemptId = beginTaskAttempt(task, assignee);
                if (taskAccessOf(task) === 'write')
                    fresh.writeLease = { taskId: task.id, attemptId, owner: assignee, acquiredAt: Date.now() };
                await writeTeam(stateRoot, fresh);
        ...
        async execute(args, exec) {
            ...
            const updated = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (identity.kind === 'captain'
                    ...
                    && task.assignee !== CAPTAIN_KEY) {
                    throw new Error(`task ${task.id} is owned by member "${task.assignee}"; call crew_reassign_task with assignee="captain" before takeover`);
                }
                ...
                if (TERMINAL_TASK_STATUSES.includes(task.status)) {
                    ...
                    if (!sameStatus || !sameOutput) {
                        throw new Error(`terminal task ${task.id} is immutable; use crew_reassign_task to retry failed/cancelled work`);
                    }
                    return {
                        task_id: task.id,
                        status: task.status,
                ...
                task.updatedAt = Date.now();
                if (TERMINAL_TASK_STATUSES.includes(task.status) && fresh.writeLease?.taskId === task.id)
                    delete fresh.writeLease;
                ...
                for (const created of followUp?.created ?? []) {
                    appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, caller.session), 'crew/task-created', {
                        teamId: fresh.id,
                        taskId: created.id,
        ...
        async execute(args, exec) {
            ...
            const prepared = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (args.from !== undefined && args.from !== from) {
                    throw new Error(`crew_send_message: "from" must be your own identity ("${from}"), not "${args.from}"`);
                }
                if (to === CAPTAIN_KEY) {
                    ...
                    appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, caller.session), 'crew/message-sent', {
                        teamId: fresh.id,
                ...
                if (fresh.halted === true) {
                    throw new Error(`team "${fresh.name}" is halted; call crew_resume before waking a member`);
                }
            ...
            if (captain !== undefined && prepared.recipient.id !== '') {
                ...
                const text = `Crew state policy: inspect ${config.stateDir}/${prepared.fresh.id}/ read-only; never edit team.json or inbox files directly. Use crew_* tools for team state.\n\n${senderText}`;
        ...
        async execute(_args, exec) {
            ...
            for (const result of quiescence) {
                if (result.status === 'rejected') {
                    ctx.logger.warn(`crew: member did not quiesce cleanly before team archive: ${String(result.reason)}`);
                }
            }
            await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'crew/team-deleted', {
                    teamId: fresh.id,
                });

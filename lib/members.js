 ...
 * (updating team state through the `crew_*` tools), and becomes idle
 ...
 * which the captain reads through `crew_status`.
 * @module dsh-crew/members
 */
...
/** Captain-only Crew tools hidden from newly spawned members. */
...
const MEMBER_LABEL_PREFIX = 'crew:';
    ...
    ctx.subagents.registerContinuableSetup((childCtx) => {
        ...
        if (selection === undefined) {
            ...
            if (descriptor.agentProvider !== durableMember?.provider || descriptor.agentModel !== durableMember?.model) {
                throw new Error(`crew: saved model route for member "${memberName}" does not match its subagent descriptor`);
            }
        }
        ...
        const disposeFallback = childCtx.on('agent/request-error', async (payload) => {
            ...
            if (separator > 0) {
                ...
                void updateFallbackState(join(workspace, stateDir), teamId, memberName, fallback, ctx).catch((error) => {
                    ctx.logger.warn(`crew: failed to persist fallback route: ${String(error)}`);
                });
            }
            ctx.logger.warn(`crew: member ${child.id} switching to fallback ${fallback.provider}/${fallback.model} after ${payload.failure.code}`);
            return { kind: 'retry' };
        });
        return () => {
            disposeFallback();
...
export function memberPersona(team, member, stateDir, executionPrompt) {
    ...
    return `You are ${member.name}, a member of the multi-agent team "${team.name}" running inside DeepSeek Harness Crew. The captain leads the team; you are a worker member${member.role ? ` with the role: ${member.role}` : ''}.
...
${injectedPrompt}
`}- The team state lives under ${stateDir}/${team.id}/ (team.json and inbox/*.jsonl). You may inspect these files read-only for diagnostics, but never edit them directly; use the crew_* tools so JSON escaping and concurrent updates stay safe.
...
Working rules:
1. When you receive a task assignment, call crew_claim_task with the task id. Keep the returned attempt_id: include it in every crew_update_task call for that execution attempt. Then mark the task in_progress.
...
4. Send a short report to the captain with crew_send_message (to=captain) when you complete a task or hit a blocker.
5. To ask a teammate something, use crew_send_message with to=<teammate name>; the message lands in their mailbox and wakes them directly — teammates talk to each other without the captain in the loop. The same applies to the captain (to=captain).
...
export async function spawnMember(ctx, config, selections, llmSelection, captain, team, member, stateDir, signal) {
    ...
    if (provider === undefined) {
        throw new Error(`crew: no subagent provider "${config.provider}" is registered (available: ${ctx.subagents.list().join(', ') || 'none'}) — `
            + 'check that the subagent provider row (e.g. subagent-spawn) is mounted in the composition');
    }
    if (provider.prepareContinuable === undefined) {
        throw new Error(`crew: provider "${config.provider}" does not support continuable members`);
    }
    if (!provider.capabilities.persona) {
        throw new Error(`crew: provider "${config.provider}" cannot apply a member persona`);
    }
    if (!provider.capabilities.toolFilter) {
        throw new Error(`crew: provider "${config.provider}" cannot restrict captain-only tools for members`);
    }
...
export async function deliverToMember(ctx, captain, childId, text, signal) {
    try {
        ...
        ctx.logger.warn(`crew: followup to member ${childId} failed: ${String(error)}`);
        return false;
    }
}
/**
...
export function interruptMember(ctx, captain, childId) {
    try {
        ...
        ctx.logger.warn(`crew: interrupt of member ${childId} failed: ${String(error)}`);
    }
}
/**
 ...
 * Crew index therefore rejects `followup()` before it can cold-resume a
    ...
    ctx.effect(() => {
        ...
        const guardedFollowup = async (parent, childId, content, options) => {
            ...
            if (retired.has(childId)) {
                throw new SubagentError(`Crew member "${childId}" was retired and cannot be resumed`, 'NOT_RESUMABLE');
            }

 ...
 *   member name), mirroring the Claude Code Agent Teams mailbox layout
 *
...
export { buildCoverageMatrix, canDeclareDelivery, classifyChangedPath, collectChangedPaths, defaultQualityDeliveryGraph, describeQualityLoop, evaluateQualityCompletion, hasValidQualityTaskFields, isQualityKind, pathMatchesScope, planQualityFollowUp, qualityPlanningPrompt, resumeTeamState, sanitizeReviewAcceptance, sanitizeReviewObjective, taskKindOf, taskAccessOf, validateCreateTask, } from "./quality-gates.js";
...
/** Durable deny-list for Crew members that must never be resumed. */
...
export async function readTeam(stateRoot, teamId) {
    try {
        ...
        if (team === undefined) {
            throw new Error(`invalid Crew state in team "${teamId}"`);
        }
        return team;
    }
    catch (error) {
...
export async function readRetiredMemberIds(stateRoot) {
    try {
        ...
        if (!Array.isArray(parsed) || parsed.some(value => typeof value !== 'string' || value === '')) {
            throw new Error('invalid Crew retired member index');
        }

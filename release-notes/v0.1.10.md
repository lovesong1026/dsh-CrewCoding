...
This release makes the Crew activity panel easier to place and size, fixes the `/crew` conversation history, and gives the whale roles a cleaner visual treatment. It is tested against DeepSeek Harness `0.1.0-rc.8`.
...
- Preserved the exact `/crew ...` input as the normal user message in the main conversation.
...
本次更新重点改善 Crew 活动面板的摆放与尺寸体验，修复 `/crew` 在主会话中的消息展示，并重新整理鲸鱼角色头像的视觉层级。已基于 DeepSeek Harness `0.1.0-rc.8` 完成验证。
...
- 将用户输入的完整 `/crew ...` 原文保留为主会话中的普通用户消息。
- 上下文注入按时间顺序显示在用户消息下方。

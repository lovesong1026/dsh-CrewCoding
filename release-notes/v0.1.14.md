...
This release adds a review-first team workflow, stronger delivery gates, and safer controls for long-running Crew sessions.

## New

- **Review plans before execution**: normal `/crew` runs now stage the roster and task DAG without creating members or scheduling work. Reviewers can edit members, prompts, models, reasoning levels, tasks, and dependencies before launch.
...
- **执行前审查计划**：普通 `/crew` 会先生成可编辑的成员阵容和任务 DAG，不创建成员、不调度任务。启动前可调整成员、提示词、模型、推理等级、任务和依赖。

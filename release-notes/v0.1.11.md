...
This release restores Crew activity and member history after a DeepSeek Harness client or host restart. It fixes the rc.8 compatibility regressions reported in [#60](https://github.com/NanmiCoder/dsh-crew/issues/60) and [#61](https://github.com/NanmiCoder/dsh-crew/issues/61).

## Fixes

- **Cold-start activity recovery**: selecting a captain session now performs a lightweight live and archived team discovery pass, so older sessions without an Crew conversation card restore their activity panel after restart.
...
- **Persistent member history**: retired Crew members remain discoverable in the Harness subagent catalog for transcript review while direct follow-up remains blocked.
...
```sh
dsh plugin --profile web add @nanmicoder/dsh-crew
```
...
本次更新修复 DeepSeek Harness 客户端或 host 重启后 Crew 活动面板与成员历史消失的问题，对应 [#60](https://github.com/NanmiCoder/dsh-crew/issues/60) 和 [#61](https://github.com/NanmiCoder/dsh-crew/issues/61) 中报告的 rc.8 兼容性回归。

## 修复

- **冷启动活动恢复**：选择队长会话时会执行一次轻量的活动团队与历史归档发现；即使旧会话没有 Crew 对话卡片，重启后也能恢复活动面板。
- **可靠的成员活动状态**：从持久化团队成员表和 Agent registry 解析真实的 `running / idle / ready`，不再依赖容易变化的 `listChildren()` 投影结构。
- **成员历史持续可见**：已退休的 Crew 成员继续保留在 Harness 子代理目录中供历史记录查看，但直接 follow-up 仍会被拒绝。

#!/usr/bin/env node
/**
 * Offline smoke verification for dsh-crew.
 *
...
import { parseCrewCreateArgs } from '../lib/client/crew-card-definition.js'
import {
  CREW_LOCALE_NAMESPACE,
  en as crewEn,
  zh as crewZh,
...
console.log('dsh-crew offline verification')
...
const profilePersona = memberPersona({ name: 'Demo', id: 'demo', description: 'goal', profile: { name: 'demo', protocol: 'p'.repeat(600) }, captainSessionId: 'c', createdAt: 0, members: [], tasks: [], taskSeq: 0 }, { name: 'Implementer', id: 'm', role: 'builder', joinedAt: 0, status: 'idle' }, '.crew')
...
const assignment = assignmentPrompt({ taskId: 't1', memberName: 'Implementer', memberId: 'm', attempt: 1, attemptId: 'a', subject: 'x', dependencyOutputs: [], executionPrompt: injected }, '.crew', 'demo')
check('execution prompt is injected into persona and assignment', assignment.includes(injected) && memberPersona({ name: 'Demo', id: 'demo', description: 'goal', captainSessionId: 'c', createdAt: 0, members: [], tasks: [], taskSeq: 0 }, { name: 'Implementer', id: 'm', role: 'builder', joinedAt: 0, status: 'idle', executionPrompt: injected }, '.crew').includes(injected))
...
const crewCardCss = await readFile(new URL('../src/client/CrewCard.module.css', import.meta.url), 'utf8')
const crewCardSource = await readFile(new URL('../src/client/CrewCard.tsx', import.meta.url), 'utf8')
...
const localeKeys = Object.keys(crewZh).sort()
const englishLocaleKeys = Object.keys(crewEn).sort()
const placeholders = value => [...value.matchAll(/\{(\w+)\}/gu)].map(match => match[1]).sort()
check(
  'Crew locale dictionaries have identical keys and template placeholders',
  localeKeys.length > 0
    && JSON.stringify(localeKeys) === JSON.stringify(englishLocaleKeys)
    && localeKeys.every(key => JSON.stringify(placeholders(crewZh[key]))
      === JSON.stringify(placeholders(crewEn[key]))),
)
check(
  'client registers the official locale namespace on both visible slots',
  CREW_LOCALE_NAMESPACE === 'crew'
    && clientIndexSource.includes("'uiConversation'")
    && clientIndexSource.includes('ctx.uiConversation.events.register(crewCardDefinition)')
    && clientIndexSource.includes('ctx.locale.register(CREW_LOCALE_NAMESPACE, { zh, en })')
    && clientIndexSource.match(/locale:\s*CREW_LOCALE_NAMESPACE/gu)?.length === 2,
)
check(
  'slash command transcript hides the duplicate pre-message result row',
  clientIndexSource.includes('HiddenCrewCommand')
    && /name:\s*'conversation\.chat\.commandview',\s*key:\s*'crew'/u.test(clientIndexSource),
)
check(
...
check(
  ...
  toolsSource.includes("name: 'crew_edit_plan'")
    ...
    && hostSource.includes('crew_edit_plan')
    && hostSource.includes('Never inspect or edit .crew state files or plugin source code'),
)
check(
...
const artworkDir = new URL('../assets/crew/', import.meta.url)
...
check(
  ...
  !/#0b1d33/iu.test(`${activityPanelCss}\n${crewCardCss}`)
    ...
    && activityPanelCss.includes('crewUnreadPulse')
    && crewCardCss.includes('object-fit: contain'),
...
check(
  ...
  activityPanelSource.includes('if (current === undefined) return')
    ...
    && crewCardSource.includes('monitorAgentTeam(owner, data.teamId)')
    && !crewCardSource.includes('setInterval(')
    && !crewCardSource.includes('fetch('),
...
const stateRoot = await mkdtemp(join(tmpdir(), 'dsh-crew-verify-'))
try {
  const team = {
    name: 'Verify Team',
...
check(
  ...
  discoveryUrls.length === 3
    && discoveryUrls[0] === '/plugins/dsh-crew/state'
    && discoveryUrls[1]?.endsWith('?archived=1')
    && discoveryUrls[2] === '/plugins/dsh-crew/state'
...
// (e.g. a run_code-wrapped crew_create) must be discovered without a
...
check(
  ...
  JSON.stringify(parseCrewCreateArgs('{"name":" Repo Review 2W! "}'))
    === JSON.stringify({ teamId: 'repo-review-2w', name: 'Repo Review 2W!' }),
)
check('malformed create tool arguments do not create a card', parseCrewCreateArgs('{bad') === undefined)
...
check(
  ...
  captainSteered
    ...
    && captainDeliveries[0]?.content[0]?.text === 'Crew message from member alice:\n\nfinished t1',
)
check(
...
const restoreWorkspace = await mkdtemp(join(tmpdir(), 'dsh-crew-selection-'))
try {
  const restoreStateRoot = join(restoreWorkspace, '.crew')
  ...
  const coldChild = fakeChildContext({
    label: 'crew:restore-team:reviewer',
    parentSessionId: 'captain-session',
...
const atomicStateRoot = await mkdtemp(join(tmpdir(), 'dsh-crew-atomic-'))
try {
  const {

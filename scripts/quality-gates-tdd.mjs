#!/usr/bin/env node
/**
 * Forced TDD checklist for Crew quality gates (docs/quality-gates.md §9.3).
 *
...
import { applyQualityFollowUp, haltTeamWork, registerCrewTools } from '../lib/tools.js'
...

{
  ...
  try {
    ...
    if (typeof index.usageSectionText === 'function') {
      usage = index.usageSectionText('crew_resume, crew_update_task')
    }
  } catch {
    usage = ''
  }
...

{
  ...
  try {
    await call('crew_create', { name: 'Gates', description: 'tool loop' })
    await call('crew_add_member', { name: 'implementer', role: 'implementer' })
    await call('crew_add_member', { name: 'reviewer', role: 'correctness-reviewer' })

    const work = await call('crew_create_task', { subject: 'legacy work' })
    const persistedWork = (await readTeam(join(workspace, '.crew'), 'gates'))?.tasks.find((item) => item.id === work.task_id)
    check(
      'tdd.create.work-kind-remains-compatible.tool',
      persistedWork?.kind === 'work' || persistedWork?.kind === undefined,
    )

    await throwsAsync('tdd.create.implementation-requires-objective.tool', () => call('crew_create_task', {
      subject: 'impl',
      kind: 'implementation',
      acceptance: ['done'],
      inScope: ['src/a.ts'],
    ...
    await haltTeamWork({
      ctx,
      stateRoot: join(workspace, '.crew'),
      teamId: 'gates',
      captain,
      signal: exec.signal,
    })
    const before = await readTeam(join(workspace, '.crew'), 'gates')
    let createUnhalted = false
    try {
      await call('crew_create_task', { subject: 'should stay halted' })
      createUnhalted = (await readTeam(join(workspace, '.crew'), 'gates'))?.halted !== true
    } catch {
      createUnhalted = (await readTeam(join(workspace, '.crew'), 'gates'))?.halted !== true
    }
    check(
    ...
    const resumeTool = definitions.get('crew_resume')
    ...
    if (resumeTool) {
      ...
      check(
        ...
        (await readTeam(join(workspace, '.crew'), 'gates'))?.halted !== true,
      )
    }
  } finally {
    await rm(workspace, { recursive: true, force: true })

/**
 * Deterministic Crew scheduler stress and restart verification.
 *
...
import { registerCrewTools } from '../lib/tools.js'
import { readArchivedTeam, readTeam, readUnreadMailbox } from '../lib/state.js'

const workspace = await mkdtemp(join(tmpdir(), 'dsh-crew-stress-'))
...
async function completeClaimed(task, outputPrefix = 'stress') {
  ...
  const claim = await call('crew_claim_task', { task_id: task.id }, owner)
  if (task.assignee === 'captain') {
    await call('crew_update_task', { task_id: task.id, status: 'in_progress' }, owner)
...
console.log('dsh-crew complex stress verification')
runtime = mountRuntime()
try {
  await call('crew_create', { name: 'Stress Matrix', description: '8 members, 31-node DAG, injected failures and cold restart' })
  for (const name of memberNames) {
    await call('crew_add_member', { name, role: `${name}-specialist` })
  }
  ...
  const alphaClaim = await call('crew_claim_task', { task_id: 't1' }, alphaOld)
  const betaClaim = await call('crew_claim_task', { task_id: 't2' }, betaOld)
  await call('crew_update_task', { task_id: 't1', status: 'in_progress', attempt_id: alphaClaim.attempt_id }, alphaOld)
  await call('crew_update_task', { task_id: 't2', status: 'in_progress', attempt_id: betaClaim.attempt_id }, betaOld)
  ...
  const [takeover, removal] = await Promise.all([
    ...
    call('crew_remove_member', { name: 'beta' }),
  ])
  ...
  const secondTakeover = await call('crew_reassign_task', {
  ...
  const secondReuse = await call('crew_create_task', {
  ...
  for (const [index, assignee] of ['gamma', 'epsilon', 'zeta', 'eta'].entries()) {
    coldProbes.push(await call('crew_create_task', {
  ...
  const coldStaleStorm = await Promise.allSettled(openBeforeRestart.flatMap(old => (
    Array.from({ length: 12 }, () => call('crew_update_task', {
      task_id: old.id,
      status: 'completed',
  ...
  for (const name of activeNames) {
    if (await liveMember(name) === undefined) {
      await call('crew_send_message', { to: name, content: 'wake for claim-race verification' })
      await settle()
    }
    const agent = await liveMember(name)
  ...
  const herdTask = await call('crew_create_task', { subject: 'seven-way-claim-herd' })
  ...
  const herd = await Promise.allSettled(herdAgents.map(agent => (
    call('crew_claim_task', { task_id: herdTask.task_id }, agent)
  )))
  ...
  check('shutdown keeps all eight retired members catalog-visible for history',
    (await runtime.ctx.subagents.listChildren(runtime.captain.id))
      .filter(child => child.kind === 'child'
        ...
        && child.label.startsWith('crew:')).length === 8)
} finally {

...
import { haltTeamWork, registerCrewTools } from '../lib/tools.js'
import { buildActivationDirective, invokedCrewGoal, invokedCrewInvocation, installCrewGestureBoundary, profileCommandName, registerCrewCommand } from '../lib/command.js'
...
const workspace = await mkdtemp(join(tmpdir(), 'dsh-crew-lifecycle-'))
...
// A non-Crew continuable sibling must survive every team lifecycle
// operation untouched.
...
const crewRuntime = registerCrewTools(ctx, {
  stateDir: '.crew',
...
const stateRoot = join(workspace, '.crew')
const state = () => readTeam(stateRoot, teamId)
const task = async id => (await state())?.tasks.find(candidate => candidate.id === id)

console.log('dsh-crew lifecycle verification')

// ── /crew slash command and gesture boundary ───────────────────
...
registerCrewCommand(ctx, () => liveProfiles)
installCrewGestureBoundary(ctx, () => liveProfiles)
...
check('bare profile command gesture asks for the goal',
  invokedCrewInvocation([userMessage('/crew-demo-delivery')], () => liveProfiles)?.profile === 'demo-delivery'
...
try {
  ...
  const analystClaim = await call('crew_claim_task', { task_id: firstSeed.id }, analyst)
  await call('crew_update_task', { task_id: firstSeed.id, status: 'in_progress', attempt_id: analystClaim.attempt_id }, analyst)
  await call('crew_update_task', {
  ...
  check('failed upstream does not unlock the next configured stage',
    (await readTeam(stateRoot, 'profile-demo'))?.tasks[1]?.status === 'pending'
      && !deliveries.some(delivery => delivery.childId === implementer.id && String(delivery.content?.[0]?.text ?? '').includes('Implement')))
  await call('crew_reassign_task', { task_id: firstSeed.id, assignee: 'analyst', reason: 'retry after user answer' })
  const retryClaim = await call('crew_claim_task', { task_id: firstSeed.id }, analyst)
  await call('crew_update_task', { task_id: firstSeed.id, status: 'in_progress', attempt_id: retryClaim.attempt_id }, analyst)
  await call('crew_update_task', {
    task_id: firstSeed.id,
    status: 'completed',
  ...
  await call('crew_send_message', { to: 'implementer', content: 'stop and wait for a user answer' })
  ...
  const profileStatus = await call('crew_status', {})
  ...
  await call('crew_create', {
    name: 'Rejected Demo',
    description: 'plan the user will reject',
    profile: 'dynamic-delivery',
    approval: 'required',
  })
  await call('crew_create_task', { subject: 'should never run', assignee: 'analyst' })
  const discardedPlan = await crewRuntime.discardStagedTeam(captain, 'rejected-demo')
  ...
  const dynamicFirst = await call('crew_create_task', { subject: 'analyze goal', assignee: 'analyst' })
  const dynamicSecond = await call('crew_create_task', {
    subject: 'implement result',
    assignee: 'implementer',
    dependencies: [dynamicFirst.task_id],
  })
  await crewRuntime.updateStagedPlan(captain, 'dynamic-demo', {
  ...
  const obsoleteReview = await call('crew_create_task', {
  ...
  const continuedPlan = await crewRuntime.continueStagedPlanning(captain, 'dynamic-demo')
  ...
  const repeatedContinue = await crewRuntime.continueStagedPlanning(captain, 'dynamic-demo')
  check('return-to-chat is idempotent while feedback is already pending',
    repeatedContinue.alreadyWaiting === true
      && (captain.cancelCount ?? 0) === captainCancelsBeforeContinue + 1
      && captain.followups.length === captainFollowupsBeforeContinue + 1)
  const modelEditedPlan = await call('crew_edit_plan', {
    operations: [
      { action: 'update_task', task_id: dynamicSecond.task_id, dependencies: [dynamicFirst.task_id] },
      { action: 'remove_task', task_id: obsoleteReview.task_id },
      { action: 'remove_member', member_name: 'reviewer' },
  ...
  check('captain can revise the staged DAG and roster through one model-facing atomic tool',
    modelEditedPlan.status === 'staged'
      ...
      && deliveries.length === deliveriesBeforePlan)
  const approvedDynamic = await call('crew_approve', { confirmation: 'user clicked Approve & Run' })
  ...
  try {
    await call('crew_create_task', { subject: 'must stay halted' })
  ...
  const resume = await call('crew_resume', { reason: 'continue after user answer' })
  ...
  await call('crew_create', { name: 'Quality Loop', description: 'review loop' })
  await call('crew_add_member', { name: 'builder', role: 'implementer' })
  await call('crew_add_member', { name: 'critic', role: 'reviewer' })
  const impl = await call('crew_create_task', {
  ...
  try {
    await call('crew_create_task', { subject: 'impl without contract', kind: 'implementation' })
  } catch {
  ...
  const implClaim = await call('crew_claim_task', { task_id: impl.task_id }, builder)
  await call('crew_update_task', { task_id: impl.task_id, status: 'in_progress', attempt_id: implClaim.attempt_id }, builder)
  ...
  const review = await call('crew_create_task', {
  ...
  const reviewClaim = await call('crew_claim_task', { task_id: review.task_id }, criticMember)
  await call('crew_update_task', { task_id: review.task_id, status: 'in_progress', attempt_id: reviewClaim.attempt_id }, criticMember)
  ...
  await call('crew_create', { name: 'Lifecycle', description: 'adversarial DAG' })
  const addedAlpha = await call('crew_add_member', { name: 'alpha', role: 'slow implementer' })
  const addedBeta = await call('crew_add_member', { name: 'beta', role: 'researcher' })
  const addedGamma = await call('crew_add_member', { name: 'gamma', role: 'reviewer' })
  ...
  const t1 = await call('crew_create_task', { subject: 'slow branch', assignee: 'alpha' })
  ...
  const alphaClaim = await call('crew_claim_task', { task_id: t1.task_id }, alpha)
  ...
  const t2 = await call('crew_create_task', { subject: 'parallel research', assignee: 'beta' })
  const t3 = await call('crew_create_task', {
    subject: 'integration gate', assignee: 'gamma', dependencies: [t1.task_id, t2.task_id],
  })
  const betaClaim = await call('crew_claim_task', { task_id: t2.task_id }, beta)
  await call('crew_update_task', {
  ...
  const resumedBeta = await call('crew_send_message', {
  ...
  const takeover = await call('crew_reassign_task', {
  ...
  const gammaClaim = await call('crew_claim_task', { task_id: t1.task_id }, gamma)
  await call('crew_update_task', {
  ...
  const gateClaim = await call('crew_claim_task', { task_id: t3.task_id }, gamma)
  await call('crew_update_task', {
  ...
  const t4 = await call('crew_create_task', { subject: 'later-round assigned work', assignee: 'alpha' })
  ...
  const t5 = await call('crew_create_task', { subject: 'must wait behind alpha', assignee: 'alpha' })
  let busyRejected = false
  try {
    await call('crew_claim_task', { task_id: t5.task_id }, alpha)
  } catch (error) {
    busyRejected = /busy with/.test(String(error))
  }
  check('a member cannot claim a second unfinished task', busyRejected)
  await call('crew_reassign_task', {
    task_id: t5.task_id, assignee: 'captain', reason: 'close busy-check task',
  })
  await call('crew_update_task', { task_id: t5.task_id, status: 'in_progress' })
  await call('crew_update_task', { task_id: t5.task_id, status: 'completed', output: 'closed' })

  await call('crew_remove_member', { name: 'alpha' })
  ...
  for (const recoveredTaskId of [t4.task_id]) {
    ...
    const claim = await call('crew_claim_task', { task_id: recoveredTaskId }, owner)
    await call('crew_update_task', { task_id: recoveredTaskId, status: 'in_progress', attempt_id: claim.attempt_id }, owner)
    await call('crew_update_task', {
  ...
  const fallback = await call('crew_send_message', { to: 'gamma', content: 'durable fallback' })
  ...
  beta.status = 'running'
  gamma.status = 'running'
  const t6 = await call('crew_create_task', { subject: 'concurrent claim' })
  beta.status = 'idle'
  gamma.status = 'idle'
  const race = await Promise.allSettled([
    call('crew_claim_task', { task_id: t6.task_id }, beta),
    call('crew_claim_task', { task_id: t6.task_id }, gamma),
  ])
  ...
  await call('crew_update_task', { task_id: t6.task_id, status: 'in_progress', attempt_id: won.attempt_id }, winner)
  await call('crew_update_task', {
  ...
  beta.status = 'idle'
  const t7 = await call('crew_create_task', { subject: 'captain takeover', assignee: 'beta' })
  const betaTakeoverClaim = await call('crew_claim_task', { task_id: t7.task_id }, beta)
  await call('crew_update_task', {
    task_id: t7.task_id, status: 'in_progress', attempt_id: betaTakeoverClaim.attempt_id,
  }, beta)
  const captainAttempt = await call('crew_reassign_task', {
    task_id: t7.task_id, assignee: 'captain', reason: 'deadline takeover',
  })
  await call('crew_update_task', { task_id: t7.task_id, status: 'in_progress' })
  await call('crew_update_task', { task_id: t7.task_id, status: 'completed', output: 'captain result' })
  ...
  beta.status = 'idle'
  gamma.status = 'idle'
  const t8 = await call('crew_create_task', { subject: 'parallel captain takeover A', assignee: 'beta' })
  const t9 = await call('crew_create_task', { subject: 'parallel captain takeover B', assignee: 'gamma' })
  const betaParallelClaim = await call('crew_claim_task', { task_id: t8.task_id }, beta)
  const gammaParallelClaim = await call('crew_claim_task', { task_id: t9.task_id }, gamma)
  await call('crew_update_task', {
  ...
  for (const recoveredTask of recoveredParallel) {
    ...
    const claim = await call('crew_claim_task', { task_id: recoveredTask.id }, owner)
    await call('crew_update_task', {
  ...
  beta.status = 'running'
  gamma.status = 'idle'
  const snapshot = await call('crew_status', {})
  ...
  check('team shutdown keeps retired members catalog-visible for historical transcripts',
    (await ctx.subagents.listChildren(captain.id))
      .filter(child => child.kind === 'child'
        ...
        && child.label.startsWith('crew:lifecycle:')).length === 3)
  ...
  await call('crew_add_member', { name: 'valid', role: 'writer', provider: 'fake', model: 'fake-model' })
  await call('crew_add_member', { name: 'invalid', role: 'reviewer', provider: 'fake', model: 'typo-model' })
  await call('crew_create_task', { subject: 'must remain staged', assignee: 'valid' })
  const childrenBeforeRejectedApproval = children.length
  advertisedModels = ['fake-model']
  let invalidApprovalRejected = false
  try {
    await call('crew_approve', { confirmation: 'user clicked Approve & Run' })
  } catch (error) {
  ...
  await call('crew_create', { name: 'Lifecycle', description: 'second generation' })
  await call('crew_delete', {})

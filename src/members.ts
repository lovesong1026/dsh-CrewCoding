  ...
  ctx.subagents.registerContinuableSetup((childCtx) => {
    ...
    if (selection === undefined) {
      ...
      if (descriptor.agentProvider !== durableMember?.provider || descriptor.agentModel !== durableMember?.model) {
        throw new Error(
          `crew: saved model route for member "${memberName}" does not match its subagent descriptor`,
        )
      }
    }
...
export async function spawnMember(
  ...
  if (provider === undefined) {
    throw new Error(
      `crew: no subagent provider "${config.provider}" is registered (available: ${ctx.subagents.list().join(', ') || 'none'}) — `
    ...
    const guardedFollowup: typeof runtime.followup = async (parent, childId, content, options) => {
      ...
      if (retired.has(childId)) {
        throw new SubagentError(
          `Crew member "${childId}" was retired and cannot be resumed`,
          'NOT_RESUMABLE',
        )
      }

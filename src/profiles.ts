...
export function listConfiguredProfiles(
  ...
  if (keys.length > MAX_TEAM_PROFILES) {
    throw new Error(`too many Crew profiles (${keys.length}); the limit is ${MAX_TEAM_PROFILES}`)
  }
  ...
  for (const rawKey of keys) {
    ...
    if (name === '') {
      throw new Error('configured Crew profiles include an empty key')
    }
    const previous = seen.get(name)
    if (previous !== undefined) {
      throw new Error(`configured Crew profiles have duplicate key "${name}"`)
    }
...
export function formatProfilesForPrompt(
  ...
  const lines = [
    'Configured team profiles (pass profile= to crew_create):',
 ...
 */
export function parseProfileInvocation(rawInput: string): CrewInvocation {
  ...
  while (index < tokens.length) {
    ...
    if (profile !== undefined) {
      throw new Error('duplicate Crew profile flag')
    }
    profile = parsed.name
...
export function resolveTeamProfile(
  ...
  if (name === '') {
    throw new Error('Crew profile name must be a non-empty string')
  }
  ...
  if (match === undefined) {
    ...
    throw new Error(`unknown Crew profile "${name}" — configured profiles: ${shown}`)
  }
...
function normalizeListedProfile(
  ...
  if (!Array.isArray(membersRaw) || membersRaw.length === 0) {
    throw new Error(`Crew profile "${listed.name}" has no members`)
  }
...
function asProfilesRecord(
  ...
  if (typeof profiles !== 'object' || Array.isArray(profiles)) {
    throw new Error('Crew profiles must be an object map of named templates')
  }

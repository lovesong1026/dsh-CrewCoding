...
import { OPEN_PANEL_EVENT } from './CrewCard.tsx'
import { StagingPlanEditor } from './StagingPlanEditor.tsx'
import type { CrewCardData } from './crew-card-definition.ts'
import type { CrewLocaleKey, CrewTranslate } from './locales.ts'
import {
...
const TASK_STATUS_LABEL: Record<string, CrewLocaleKey> = {
...
function taskStatusLabel(status: string, t: CrewTranslate): string {
  const key = TASK_STATUS_LABEL[status]
  return key === undefined ? status : t(key)
}

function formatTaskIds(ids: readonly string[], t: CrewTranslate): string {
...
function CollapsedBadge({ count, busy, onClick, t }: {
  ...
  return (
    <button type="button" className={css.badge} data-crew-collapsed data-busy={busy} onClick={onClick} aria-label={t('activity.badgeAria', { count })}>
...
function taskSummary(team: ActivityTeam, t: CrewTranslate, discarded = false): string {
...
function ProgressOverview({ team, t, discarded = false }: { readonly team: ActivityTeam; readonly t: CrewTranslate; readonly discarded?: boolean }) {
...
function historicCardTeam(data: CrewCardData, owner: string): ActivityTeam {
  return {
    workspace: '',
    teamId: data.teamId,
    name: data.teamName,
...
export function ActivityPanel({ sessionsList, modelDirectories, openMember, t }: ActivityPanelProps) {
  ...
  const [historic, setHistoric] = useState<ReadonlyMap<string, { data: CrewCardData; owner: string }>>(new Map())
  ...
  useEffect(() => {
    const onOpenPanel = (event: Event): void => {
      ...
      setOpen(true)
      const detail = (event as CustomEvent<CrewCardData>).detail

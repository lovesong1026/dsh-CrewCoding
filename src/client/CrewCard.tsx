...
import css from './CrewCard.module.css'

/** Window event name the floater listens for to open itself. */
export const OPEN_PANEL_EVENT = 'crew:open-panel'
...
export type CrewCardProps =
  PropsRuntime<'conversation.chat.node', 'crew'>
  & PropsLocale<'crew'>
  & CrewCardInjected
...
function openActivityPanel(data: CrewCardData): void {
...
export function CrewCard({ node, openMember, sessionId, t }: CrewCardProps) {
  const data = node.data as CrewCardData
  ...
  const resolved = useMemo<CrewCardData>(() => ({
    ...data,
  ...
  return (
    <section className={css.root} data-crew-card data-team-id={resolved.teamId}>

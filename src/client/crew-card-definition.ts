...
export function parseCrewCreateArgs(value: string): { teamId: string; name: string } | undefined {
  try {
...
export const crewCardDefinition: ConversationNodeDefinition<CrewNodeState> = {
  kind: 'crew',
  target: 'chat',
  match: (event) => {
    if (event.type === 'tool/call' && event.data.name === 'crew_create') {
      return parseCrewCreateArgs(event.data.arguments) === undefined
        ? null
  ...
  start: (_context, match) => {
    if (match.event.type !== 'tool/call') {
      throw new Error('crew card start requires crew_create tool/call')
    }
    const parsed = parseCrewCreateArgs(match.event.data.arguments)
    if (parsed === undefined) throw new Error('crew card start requires valid create arguments')
  ...
  buildViewNode: (context): ChatConversationViewNode | null => {
    ...
    const state = context.state as CrewNodeState

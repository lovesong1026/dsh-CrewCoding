...
export async function assembleTeamSnapshot(
  ...
  for (const member of roster) {
    try {
      ...
      ctx.logger.warn(`crew: mailbox read failed for ${member.name}: ${String(error)}`)
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        ctx.logger.warn(`crew: skipped unreadable team state "${entry.name}" in workspace "${root.workspace}"`)
      }
    }
  }
  return snapshots
  ...
  for (const root of roots) {
    for (const teamId of await listArchivedTeamIds(root.stateRoot)) {
      try {
        ...
        ctx.logger.warn(`crew: skipped unreadable archived team "${teamId}" in workspace "${root.workspace}"`)
      }
    }
  }
  return snapshots

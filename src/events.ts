/**
 * Durable Crew session events and their emitter.
 *
...
import type { CrewEventType } from './event-types.ts'

/** Event types already reported as unsupported, to avoid repetitive logs. */
const skippedEventTypes = new Set<CrewEventType>()

/**
 * Append one Crew event to a Session, containing failures (a broken
...
export function appendTeamEvent(
  ...
  session: Session,
  type: CrewEventType,
  data: SessionEventMap[CrewEventType],
): void {
  ...
  if (known?.has(type) !== true) {
    if (!skippedEventTypes.has(type)) {
      ...
      ctx.logger.debug(`crew: session event "${type}" omitted because this harness does not recognize it`)
    }
    return
  }
  try {
    ...
    ctx.logger.warn(`crew: session record failed after ${type}: ${String(error)}`)
  }
}

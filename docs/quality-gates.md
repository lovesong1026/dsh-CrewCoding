...
把 Crew 从“可靠的多 Agent 任务调度底座”升级为“带结构化任务合同、审查结论、自动修复链、强制 TDD 和范围门禁的交付系统”。
...
| 停止整队 | `haltTeamWork()`、`POST /plugins/dsh-crew/halt` | 停止仍不删除团队 |
...
`crew_update_task` 增加可选结构化参数，或允许 `output` 旁路解析一份 JSON。优先显式参数，不要只靠自由文本：
...
| 工具 | 改动 |
|---|---|
| `crew_create_task` | 增加合同字段；halt 默认拒绝；质量 kind 强制合同；范围冲突拒绝 |
| `crew_update_task` | 增加 verdict / findings / changedPaths / acceptanceResults / commandsRun；按 kind 拒绝非法 completed |
| `crew_status` | 增加 kind、round、verdict、findings 摘要、coverage matrix、escalated、halt/resume 状态 |
| `crew_create` | profile 可带 `reviewPolicy`；快照到 `team.profile` / `team.reviewPolicy` |

### 7.2 新工具

| 工具 | 作用 |
|---|---|
| `crew_resume` | 显式恢复 halted 团队，必须带 reason |
...
- `crew/task-created` 增加 `kind?`、`round?`。
- `crew/task-updated` 增加 `verdict?`、`round?`。
- 新增 `crew/team-resumed`：`{ teamId, reason }`。
...
工具层测试通过 `registerCrewTools` 调真实 compiled tools。
...
```text
按 docs/quality-gates.md 完整实现 Crew 质量门禁与多轮审查。

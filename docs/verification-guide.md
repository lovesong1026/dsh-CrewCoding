...
> 本文从 dsh-crew 插件（多智能体团队协作 + Web UI 活动面板）的完整验证历程蒸馏而来。
...
mkdir -p ~/.dsh/profiles/crew-check/node_modules
ln -sfn /absolute/path/to/plugin ~/.dsh/profiles/crew-check/node_modules/<pkg>
cat > ~/.dsh/profiles/crew-check/package.json <<'EOF'
{ "name": "dsh-profile-check", "private": true, "dependencies": {},
  "dsh": { "profile": { "bundles": ["@deepseek-ai/dsh-base", "@deepseek-ai/dsh-web-app", "<pkg>"] } } }
EOF
printf '[]\n' > ~/.dsh/profiles/crew-check/cordis.patch.yml   # 必须是顶层数组！

dsh --profile crew-check --dump-config | grep -A 4 "id: crew"
```
...
```sh
mkdir -p /tmp/crew-e2e && cd /tmp/crew-e2e
dsh --profile headless "用 Crew 完成一个小任务：创建团队'标题方案'，加 2 个成员（alice 负责研究，bob 负责撰写），创建 2 个任务（t2 依赖 t1）分配给他们，唤醒他们完成，最后汇总产出。任务要小，每个成员只做一个简单任务。"
```
...
ls -la /tmp/crew-e2e/.crew/

# 会话日志：每个会话一个目录，成员子会话是独立 uuid 目录
ls -lt ~/.dsh/sessions/--private-tmp-crew-e2e--/

# 事件流（zstd 压缩，用 zstdcat 解压后数 crew/* 事件）
zstdcat ~/.dsh/sessions/<ws>/session-<id>/session.jsonl.zstd \
  | grep -o '"type":"crew/[^"]*"' | sort | uniq -c
...
npx -p @deepseek-ai/dsh@0.0.1-rc.1 dsh plugin --profile crew-beta add @deepseek-ai/dsh-base
npx -p @deepseek-ai/dsh@0.0.1-rc.1 dsh plugin --profile crew-beta add @deepseek-ai/dsh-web-app
npx -p @deepseek-ai/dsh@0.0.1-rc.1 dsh plugin --profile crew-beta add /abs/path/to/dsh-crew
# 启动（managed background task，保存 task id；CLI 与 bundle 同通道）：
npx -p @deepseek-ai/dsh@0.0.1-rc.1 dsh --profile crew-beta --host 127.0.0.1 --port 3081
# 看到精确 URL 后再 curl
...
print(any('crew' in e['id'] for e in g.get('entries', [])))
"
...
const task = await useOrCreateTaskSpace('crew webui test')
await openOrReuseTab('http://127.0.0.1:3081', { wait: true, timeout: 30 })

// 组件必须挂 data-* 探针属性（data-crew-activity / data-task-state / data-member-running ...）
const probe = await js(String.raw`(() => {
  const panel = document.querySelector('[data-crew-activity]')
...
```js
await captureScreenshot('/tmp/crew-panel.png')   // 返回文件路径
```
...
- [ ] dsh --profile crew-check --dump-config | grep "id: <插件>"   # 组合树含插件行
...
- [ ] 落盘：.crew 状态文件存在（或按预期归档）
- [ ] zstdcat 会话日志：crew/* 事件数量与流程一一对应

### GUI（独立 web 实例 3081 + ego-browser）
- [ ] dsh --profile crew-web --patch port.patch.yml 启动，index 200

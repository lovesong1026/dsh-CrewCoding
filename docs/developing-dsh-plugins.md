...
> 本文是 dsh-crew 插件（host 工具 + 浏览器活动面板 + 对话流卡片）开发全过程的经验蒸馏。
> 覆盖 bundle 插件从骨架、host 面、client 面、构建安装到踩坑修复的完整流程，供 coding agent 直接照做。
> 参考实现：`dsh-crew`（成品）、DSH 仓库 `packages/workflow/tool-workflow`（工具插件模板）、
...
export const Config: z<Config> = z.object({ stateDir: z.string().default('.crew') })
...
```ts
// 团队状态 = workspace 下 .crew/<teamId>/team.json + inbox/*.jsonl
...
export interface CrewTeamCreatedData { readonly teamId: string; readonly name: string }
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap { 'my-plugin/team-created': CrewTeamCreatedData }
}
```
...
npx -p @deepseek-ai/dsh dsh plugin --profile web add /absolute/path/to/dsh-crew
